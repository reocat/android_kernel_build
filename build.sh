...
#     if defined, skip compiling anything if the kernel verison in vmlinux
#     matches the expected kernel version. This is useful for mixed build, where
#     GKI kernel does not change frequently and we can simply almost everything
#     in build.sh
#
#   PRE_DEFCONFIG_CMDS
...
#
#   GKI_BUILD_CONFIG
#     If set, builds a second set of kernel images using GKI_BUILD_CONFIG.
#     GKI_DIST_DIR (above) is set and the GKI kernel's DIST output is copied
#     to this DIST output. Any variables prefixed with GKI_ are passed into
#     into the second build.sh invocation
#
...
if [ -n "${GKI_BUILD_CONFIG}" ]; then
  ...
  # Inherit SKIP_MRPROPER unless overriden by GKI_SKIP_MRPROPER
  ...
  GKI_ENVIRON+=" $(printenv | sed -n -E -e 's/^GKI_([^=]+=.*)$/\1/p' | tr '\n' ' ')"
...
if [ -n "${SKIP_IF_VERSION_MATCHES}" ]; then
  if [ -e "${DIST_DIR}/vmlinux" ]; then
    kernelversion="$(cd ${KERNEL_DIR} && make -s "${TOOL_ARGS[@]}" O=${OUT_DIR} kernelrelease)"
    if [[ ! "$kernelversion" =~ ".*dirty.*" ]] && \
       strings ${DIST_DIR}/vmlinux | grep -q "Linux version ${kernelversion}"; then
      echo "========================================================"
...
[ -n "${GKI_DIST_DIR}" ] && cp -r ${GKI_DIST_DIR}/* ${DIST_DIR}/
