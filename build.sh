...
#
#   WHITELIST_STRICT_MODE
#     if defined, add a build-time check between the KMI_WHITELIST and the
#     effective kernel (and in-tree modules) KMI resulting from the build.
#
...
if [ -n "${KMI_WHITELIST}" ]; then
  ...
  pushd $ROOT_DIR/$KERNEL_DIR
    ...
    elif [ -n "${WHITELIST_STRICT_MODE}" ]; then
      echo "ERROR: WHITELIST_STRICT_MODE requires TRIM_NONLISTED_KMI=1" >&2
      exit 1
    fi
  ...
  echo "ERROR: WHITELIST_STRICT_MODE requires a KMI_WHITELIST" >&2
  exit 1
fi
...
if [ -n "${WHITELIST_STRICT_MODE}" ]; then
  echo "========================================================"
  echo " Comparing the KMI and the whitelists:"
  ${ROOT_DIR}/build/abi/compare_wl "${OUT_DIR}" "${OUT_DIR}/abi_whitelist.raw"
fi
