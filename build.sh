...
if [ -n "${GKI_BUILD_CONFIG}" ]; then
  ...
  if [[ "${MAKE_GOALS}" =~ Image*|vmlinux ]]; then
...
if [ -z "${SKIP_MRPROPER}" ] ; then
  set -x
  (cd ${KERNEL_DIR} && make "${TOOL_ARGS[@]}" O=${OUT_DIR} ${MAKE_ARGS[@]:+"${MAKE_ARGS[@]}"} mrproper)
  set +x
fi
...
if [ -z "${SKIP_DEFCONFIG}" ] ; then
  set -x
  (cd ${KERNEL_DIR} && make "${TOOL_ARGS[@]}" O=${OUT_DIR} ${MAKE_ARGS[@]:+"${MAKE_ARGS[@]}"} ${DEFCONFIG})
  set +x
...
if [ "${LTO}" = "none" -o "${LTO}" = "thin" -o "${LTO}" = "full" ]; then
  ...
  (cd ${OUT_DIR} && make "${TOOL_ARGS[@]}" O=${OUT_DIR} ${MAKE_ARGS[@]:+"${MAKE_ARGS[@]}"} olddefconfig)
  set +x
elif [ -n "${LTO}" ]; then
...
set -x
(cd ${OUT_DIR} && make O=${OUT_DIR} "${TOOL_ARGS[@]}" ${MAKE_ARGS[@]:+"${MAKE_ARGS[@]}"} ${MAKE_GOALS})
set +x
...
if [ -n "${BUILD_INITRAMFS}" -o  -n "${IN_KERNEL_MODULES}" ]; then
  ...
  (cd ${OUT_DIR} &&                                                           \
   make O=${OUT_DIR} "${TOOL_ARGS[@]}" ${MODULE_STRIP_FLAG}                   \
        INSTALL_MOD_PATH=${MODULES_STAGING_DIR} ${MAKE_ARGS[@]:+"${MAKE_ARGS[@]}"} modules_install)
fi
...
if [[ -z "${SKIP_EXT_MODULES}" ]] && [[ -n "${EXT_MODULES}" ]]; then
  ...
  for EXT_MOD in ${EXT_MODULES}; do
    ...
    make -C ${EXT_MOD} M=${EXT_MOD_REL} KERNEL_SRC=${ROOT_DIR}/${KERNEL_DIR}  \
                       O=${OUT_DIR} "${TOOL_ARGS[@]}" ${MAKE_ARGS[@]:+"${MAKE_ARGS[@]}"}
    make -C ${EXT_MOD} M=${EXT_MOD_REL} KERNEL_SRC=${ROOT_DIR}/${KERNEL_DIR}  \
                       O=${OUT_DIR} "${TOOL_ARGS[@]}" ${MODULE_STRIP_FLAG}    \
                       INSTALL_MOD_PATH=${MODULES_STAGING_DIR}                \
                       ${MAKE_ARGS[@]:+"${MAKE_ARGS[@]}"} modules_install
    set +x
  done
...
for ODM_DIR in ${ODM_DIRS}; do
  ...
  if [ -d ${OVERLAY_DIR} ]; then
    ...
    make -C ${OVERLAY_DIR} DTC=${OUT_DIR}/scripts/dtc/dtc                     \
                           OUT_DIR=${OVERLAY_OUT_DIR} ${MAKE_ARGS[@]:+"${MAKE_ARGS[@]}"}
...
if [ -z "${SKIP_CP_KERNEL_HDR}" ]; then
  ...
  make -C ${OUT_DIR} O=${OUT_DIR} "${TOOL_ARGS[@]}"                           \
          INSTALL_HDR_PATH="${KERNEL_UAPI_HEADERS_DIR}/usr" ${MAKE_ARGS[@]:+"${MAKE_ARGS[@]}"}      \
          headers_install
