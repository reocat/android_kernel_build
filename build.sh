...
if [[ -n "${IS_GKI_BUILD}" ]] && [[ -s "${OUT_DIR}/modules.order" ]]; then
  echo "========================================================"
  echo " Checking the list of GKI modules:"
  set +e
  diff -u "${GKI_MODULES_ORDER}" "${OUT_DIR}/modules.order"
  if [ $? -ne 0 ]; then
...
if [ -f "${GKI_MODULES_ORDER}" ]; then
  rev "${GKI_MODULES_ORDER}" | cut -d'/' -f1 | rev > "${DIST_DIR}/gki_modules.list"
fi
