...
function create_reduced_modules_order() {
	...
	local staging_dir=${2}/ramdisk_staging
	...
	(
		set +x
		cd ${src_dir}
		rm -f ${modules_staging_dir}/modules.order
		...
		done < ${src_dir}/modules.order

		# External modules
		mkdir -p ${modules_staging_dir}/extra
		for ko in $(find ${src_dir}/extra/. -name "*.ko"); do
			if grep -q $(basename ${ko}) ${modules_list_file}; then
				mkdir -p ${modules_staging_dir}/extra
				cp -p extra/$(basename ${ko}) ${modules_staging_dir}/extra/$(basename ${ko})
