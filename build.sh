...
function create_modules_staging() {
  ...
  if [ -z "${skip_trim_modules}" ]; then
    ...
    if [ -f ${dest_dir}/modules.blocklist ]; then
      # Future: the modules blocklist could contain module aliases instead of the filename
    ...
    (
      cd ${dest_dir}
      find * -type f -name "*.ko" | grep -v -w -f modules.order $used_blocklist_modules | xargs -r -t -n 1 rm
    )
  fi
