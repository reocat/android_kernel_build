...
if [ -n "${MODULES}" ]; then
  ...
  if [ "${BUILD_INITRAMFS}" = "1" ]; then
    ...
    if [ -n "${MODULES_CHARGER_LIST}" ]; then
      cp ${MODULES_ROOT_DIR}/modules.load.charger ${DIST_DIR}/modules.load.charger
    fi
    if [ -n "${BUILD_VENDOR_BOOT_IMG}" ]; then
      ...
      if [ -n "${MODULES_CHARGER_LIST} "]; then
        cp ${MODULES_ROOT_DIR}/modules.load.charger ${DIST_DRI}/vendor_boot.modules.load.charger
      fi
      ...
      if [ -n "${MODULES_CHARGER_LIST}" ]; then
        cp ${MODULES_ROOT_DIR}/modules.load.charger ${DIST_DIR}/vendor_kernel_boot.modules.load.charger
      fi
    fi
