...
#
#   BUILD_GKI_ARTIFACTS
#     if defined, build a boot-img-${BUILD_NUMBER}.tar.gz archive that contains
#     several GKI boot-*.img files with different kernel compression format.
...
#     ${DIST_DIR}, e.g., Image, Image.gz, Image.lz4, etc. Individul
...
#     - MKBOOTIMG_PATH=<path to the mkbootimg.py script which builds boot.img>
#       (defaults to tools/mkbootimg/mkbootimg.py)
#     - BOOT_IMG_SIZE=<The size of the boot-${kernel_version}.img to build>
#       This is required, and the file ${DIST_DIR}/Image must exist.
#     - BOOT_IMG_GZ_SIZE=<The size of the boot-${kernel_version}-gz.img to
#       build> This is only required when ${DIST_DIR}/Image.gz is present.
#     - BOOT_IMG_LZ4_SIZE=<The size of the boot-${kernel_version}-lz4.img to
#       build> This is only required only when ${DIST_DIR}/Image.lz4 is present.
#     - BOOT_IMG_<COMPRESSION>_SIZE=<The size of the
#       boot-${kernel_version}-${compression}.img to build>. This is only
#       required when ${DIST_DIR}/Image.${compression} is present.
#     - BUILD_NUMBER=<The boot image archive to build:
#       boot-img-${BUILD_NUMBER}.tar.gz> If BUILD_NUMBER is absent, the
#       output archive name will be boot-img.tar.gz.
