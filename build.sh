...
#     effectively unsets CC to fall back to the default gcc detected by the
#     build. To use a custom gcc from PATH, use the an absolute path, e.g.
...
# CC=gcc is effectively a fallback to the default detected gcc. If the user
# wants to use a custom compiler, they are still able to pass an absolute path
# CC=/usr/bin/gcc.
[ "${CC}" == "gcc" ] && unset CC
