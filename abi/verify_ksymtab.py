...
def main():
  ...
  parser.add_argument(
      "--symvers-file",
      required=True,
      help="Module.symvers or vmlinux.symvers file to check ksymtab",
  )
  ...
  parser.add_argument(
      ...
      default=["vmlinux"],
      help=(
          "Objects whose exports to consider when building ksymtab for"
          " verification"
      ),
  )
  ...
  with open(args.symvers_file) as symvers_file:
    for line in symvers_file:
      ...
      # 0= CRC; 1=symbol; 2=object name; 3=export type; 4=Namespace(optional)
      if ksym[3].startswith("EXPORT_SYMBOL") and ksym[2] in args.objects:
