...
//  Dump symbols in the __LKA_DEFINE section of a 64 bit ARM64 little-endian
...
const char *cmd;
const char *file;
bool lka_dump_debug = false;
bool lka_dump_reloc = false;

#define LKA_V_PREFIX 		"__lka_v_"
#define	LKA_V_PREFIX_LEN	8

void usage()
{
	fprintf(stderr, "usage: %s [-e] file.o\n"
			"\n"
			"Dumps values of symbols in LKA_DEFINE section in\n"
...
rela_t *relatab_alloc(section_t *relalka, size_t *nrelatab, void *map)
{
	if (relalka == NULL) {
		*nrelatab = 0;
		return NULL;
...
void relatab_free(section_t *relalka, rela_t *relatab, void *map)
{
	if (relalka == NULL)
		return;

	rela_t *lkarelatab = (rela_t *) ((uintptr_t) map + relalka->sh_offset);
	if (lkarelatab != relatab)
		free(relatab);
}
...
void dump(section_t *lka, section_t *relalka, u32_t lkaix,
	  ...
	  size_t nrelatab, print_t print, void *map)
{
	...
	symbol_t *symbol = (symbol_t *)((uintptr_t) map + symtab->sh_offset);
	symbol_t *symbolend = symbol + n;
	unsigned char *data = (unsigned char *) map + lka->sh_offset;
	...
	for (symbol_t *s = symbol; s < symbolend; ++s) {
		...
		if (!lka_dump_reloc && relatab_has_address(relatab,
							   nrelatab, adr))
			continue;
...
{
	printf("enum __lka_%s_s { __lka_%s_size = %ld };\n",
	...
	case 0:
		return;
	case 1:
		printf("enum __lka_%s_v { __lka_%s_val = 0x%02x };\n",
		       name, name, *(unsigned char *) value);
		return;
	case 2:
		printf("enum __lka_%s_v { __lka_%s_val = 0x%04x };\n",
		       name, name, *(u16_t *) value);
		return;
	case 4:
		printf("enum __lka_%s_v { __lka_%s_val = 0x%08x };\n",
		       name, name, *(u32_t *) value);
		return;
	case 8:
		printf("enum __lka_%s_v { __lka_%s_val = 0x%016lx };\n",
	...
	for (p = value; p < endp; ++p, ++i)
		printf("enum __lka_%s_v_%08x { __lka_%s_val_%08x = 0x%02x };\n",
		       name, i, name, i, *p);
}
...
int main(int argc, char **argv)
{
	...
	lka_dump_debug = getenv("LKA_DUMP_DEBUG") != NULL;
	lka_dump_reloc = getenv("LKA_DUMP_RELOC") != NULL;
	char *arg0 = argv[0];
	...
	section_t *lka = NULL;
	section_t *relalka = NULL;
	section_t *symtab = NULL;
	u32_t lkaix = 0;

	for (section_t *s = section; s < section_end; ++s) {
		section_validate(s, length, shstrtab, header->e_shnum);
		if (!strcmp(section_name(s, shstrtab, map), "LKA_DEFINE")){
			if (lka)
				errexit("multiple LKA_DEFINE sections");
			lka = s;
			lkaix = s - section;
		}
		if (!strcmp(section_name(s, shstrtab, map), ".relaLKA_DEFINE")){
			if (relalka)
				errexit("multiple .relaLKA_DEFINE sections");
			relalka = s;
		}
	...
	if (!lka)
		errexit("no LKA_DEFINE section");
	if (!symtab)
		errexit("no symbol table section");

	if (relalka) {
		if (!(relalka->sh_flags & SHF_INFO_LINK))
			errexit(".relaLKA_DEFINE not linked to LKA_DEFINE");
		if (lkaix != relalka->sh_info)
			errexit(".relaLKA_DEFINE linked to wrong section");
	}
	if (lka_dump_debug)
		printf("lkaix = 0x%x\n", lkaix);
	...
	size_t nrelatab;
	rela_t *relatab = relatab_alloc(relalka, &nrelatab, map);

	dump(lka, relalka, lkaix, symtab, strtab, relatab, nrelatab, fp, map);

	relatab_free(relalka, relatab, map);
