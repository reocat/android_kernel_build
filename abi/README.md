...
(`CONFIG_MODVERSIONS`) as an additional mechanism to enforce KMI at runtime.
Module versioning can cause CRC mismatch failures at module load time if the
KMI is broken by a module. For example, here is a module load failure due to
a CRC mismatch for the `module_layout()` symbol:
...
2. It adds a run time check to avoid accidentally loading a module that’s not
...
3. `abidiff` has some current limitations in identifying ABI differences in
   certain convoluted cases (they are being fixed) that `CONFIG_MODVERSIONS`
   can catch.

As an example of (1) above, consider the [fwnode
](https://android.googlesource.com/kernel/common/+/refs/heads/android-mainline/include/linux/device.h#598)
field in [struct device
](https://android.googlesource.com/kernel/common/+/refs/heads/android-mainline/include/linux/device.h#535).
That field MUST be opaque to modules so that they can’t make changes to fields
...
can then make changes to `device->fwnode->dev` or `device->fwnode->ops`. That’s
...
2. If a future kernel update changes the `struct fwnode_handle` (the data type
   ...
   cannot be captured in binary representation as of now.

Having module versioning enabled prevents all of these issues.

### How to check for CRC mismatch without booting the device?

Releases of `abidiff` in the near future will add support for checking CRC
values between kernels so that you can catch these differences at the same time
you catch other KMI differences.
...
modules. Each line consists of the CRC value, symbol name, vmlinux/module name
exporting the symbol and export type (EXPORT\_SYMBOL vs EXPORT\_SYMBOL\_GPL).
...
If you don’t have all the build artifacts, but just have the vmlinux file of
...
3. That `.c` file will have a corresponding `.symtypes` file in the GKI and
   your kernel built artifacts.
    ...
    c. No ’#’ prefix in the start of the line means the symbol is a function.
       For example:
...
If one kernel keeps a symbol/data type to be opaque to the modules and the
...
That’ll cause a lot of CRC mismatches and one of them would be for
`module_layout()`. If you compare the `module.symtypes` for that symbol, it’ll
look like this:
...
because one of the files in your kernel added a `#include` that isn’t present
in the GKI kernel.

To find and fix this, follow these steps:

1. Go to the header file that defines the symbol/data type having this
...
4. Now compile your module. You’ll get a build time error that shows the chain
...
6. Once you’ve identified the change, revert it in your kernel or [upload it to
   ACK and get it merged](https://android.googlesource.com/kernel/common/+/refs/heads/android-mainline/README.md).
...
That’ll cause a lot of CRC mismatches, but one of them would be for
...
2. If it’s a straight forward symbol difference between your kernel and the GKI
...
3. Sometimes a symbol is deleted in a tree and you also want to delete it in
  ...
  b. **NOTE:** Do not copy-paste tabs

  c. You’ll get a short list of commits. The first one is probably what you
     want. Otherwise, go through the list till you find the commit.

4. Once you’ve identified the change, revert it in your kernel or [upload it
   to ACK and get it merged](https://android.googlesource.com/kernel/common/+/refs/heads/android-mainline/README.md).
