...
automated:
 ...
 6. Working with symbol allow-lists
...
comparison with KMI (Kernel Module Interface) allow-lists. These are simple
plain text files that list relevant ABI kernel symbols. E.g. a allow-list file
...
for details about the KMI allow-list file format.

Changes to other ELF symbols would not be considered any longer unless they are
indirectly affecting symbols that are allowed. An allowed-list file can be
...
certain level of organization, additional allow-list files can be specified by
using `ADDITIONAL_KMI_WHITELISTS=` in the `build.config`. Similarly, it refers
to allow-lists in the `$KERNEL_DIR` and multiple files need to be separated by
whitespaces.

In order to **create an initial allow-list or to update an existing one**, the
...
exported it is not actively used (i.e. required) by any module. The allow-list
would therefore contain `func1` and `func2` only.

`extract_symbols` offers a flag to update an existing or create a new allow-list
based on the above analysis: `--whitelist <path/to/abi_allow_list>`.

In order to update an existing allow-list based on a built Kernel tree, run
...
updates the allow-lists for the GKI aarch64 Kernel.

```
  (build the kernel)
  $ BUILD_CONFIG=common/build.config.gki.aarch64 build/build.sh

  (update/create the allow-list)
  $ build/abi/extract_symbols out/android-mainline/dist --whitelist common/abi_gki_aarch64
```
...
takes a path to a KMI allow-list file:

```
  $ dump_abi --linux-tree path/to/out --out-file /path/to/abi.xml --kmi-whitelist /path/to/allow_list
```
...
Hence, `--kmi-whitelist path/to/kmi_allow_list` can be used to limit that
comparison to allowed symbols by passing a KMI allow-list.
