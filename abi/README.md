...
```
    $ diff_abi \
        --baseline out/android-mainline/dist/abi-<id>.xml \
        --new out/android-mainline/dist/abi-<id>-dirty.xml \
        --report abi-report
```

and will report an ABI difference similar to this:
...
### How to fix a broken ABI

If you didn't intentionally break the kernel ABI, then you need to investigate
via the Gerrit test log to identify the issue(s) reported by the tool. Most
...
Some kernel branches might come with golden ABI representations as part of their
source distribution. These ABI representations are supposed to be accurate and
should reflect the result of `build_abi.sh` as if you would execute it on your
own. As the ABI is heavily influenced by various kernel configuration options,
these .xml files usually belong to a certain configuration.<br>
