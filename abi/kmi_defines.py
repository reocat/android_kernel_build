...
def usage() -> None:  # TODO(pantin): use argparse and logging
...
def path_is_relative(path: str) -> bool:
    """If path is relative return True, False otherwise."""
...
class KernelModule:
    ...
    def __init__(self, kofile: str) -> None:
        """Contruct a KernelModule object."""
        ...
        self.cmd_file = os.path.join(self.directory, "." + self.base + ".cmd")
        ...
        #       os.path.join(self.rel_dir, kofile_name + ".o")

        kofile_name, _ = os.path.splitext(self.base)
        objs = shell_line_to_o_files_list(self.cmd_text)
        objs.sort()
        expected = [  # sorted, i.e.: .mod.o < .o
            os.path.join(self.rel_dir, kofile_name + ".mod.o"),
            os.path.join(self.rel_dir, kofile_name + ".o")
        ]
        if objs != expected:
            raise StopError("unexpected .o files in: " + self.cmd_file)
    ...
    def get_build_dir(self) -> str:
        ...
        if not self.file.endswith(self.rel_file):
            raise StopError("could not find: " + self.rel_file +
                            " at end of: " + self.file)
        index = len(self.file) - len(self.rel_file)
        if index > 0 and self.file[index - 1] == "/":
            index -= 1
    ...
    def get_object_files(self, build_dir: str) -> List[str]:
        ...
        The ocmd_file is the file with extention ".o.cmd" (see below).
...
class Kernel:
    ...
    def __init__(self, kernel: str) -> None:
        ...
        libs = os.path.join(self.build_dir, "vmlinux.libs")
        objs = os.path.join(self.build_dir, "vmlinux.objs")
        file_must_exist(libs)
        file_must_exist(objs)
        contents = readfile(libs)
        aolist = contents.split()
...
class KernelComponentCreationError(KernelComponentBase):  # pylint: disable=too-few-public-methods
    ...
    def get_error(self) -> str:
        """Return the error."""
        return self.filename + ": " + self.error
...
class KernelComponent(KernelComponentBase):
    ...
    def __init__(self, filename: str) -> None:
        ...
        if filename.endswith("vmlinux.o"):
            self.kernel = True
            self.kind = Kernel(filename)
        else:
            self.kernel = False
            self.kind = KernelModule(filename)
        self.build_dir = self.kind.get_build_dir()
        self.source_dir = self.get_source_dir()
        self.files_o = self.kind.get_object_files(self.build_dir)
    ...
    def init_build_data(self) -> None:
        ...
        - store in self.deps_set the set of headers included by the files
          in self.files.o;
        ...
        for obj in self.files_o:
            ...
            cc_line = makefile_assignment_right(line)
            ...
            for dep in dependendencies:
                if path_is_relative(dep):
                    dep = os.path.join(self.build_dir, dep)
                dep = os.path.realpath(dep)
                depends.append(dep)
                deps_set.add(dep)

            if path_is_relative(src):
                src = os.path.join(self.build_dir, self.source_dir)
            src = os.path.realpath(src)

            self.targets.append(Target(obj, src, cc_line, depends))
...
def work_on_all_components() -> List[KernelComponentBase]:
    ...
    with multiprocessing.Pool(os.cpu_count()) as pool:
        components = pool.map(kernel_component_factory, all_ko_and_vmlinux())
    return components
...
def main() -> int:
    ...
    comp = kernel_component_factory(file)
    error = comp.get_error()
    if error != "":
        logging.error(error)
        return 1
