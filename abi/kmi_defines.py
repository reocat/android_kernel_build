...
class KernelModule:
  ...
  def __init__(self, kofile: str) -> None:
    ...
    self._cmd_file = os.path.join(self._directory, "." + self._base + ".cmd")
  ...
  def get_build_dir(self) -> str:
    ...
    if not self._file.endswith(self._rel_file):
      raise StopError(
          "could not find: " + self._rel_file + " at end of: " + self._file
      )
...
class Target:  # pylint: disable=too-few-public-methods
  ...
  def __init__(self, obj: str, src: str, cc_line: str, deps: List[str]) -> None:
    self._obj = obj
    self._src = src
    self._deps = deps
    ...
    if (
      ...
      raise StopError(
          "unexpected or missing arguments for: " + obj + " cc_line: " + cc_line
      )
    ...
    def verify_file(
        file: str, index: int, kind: str, cc_list: List[str], target_file: str
    ) -> None:
...
def kernel_component_factory(filename: str) -> KernelComponentBase:
  ...
  try:
    ...
    return KernelComponentCreationError(filename, " ".join([*stop_error.args]))
...
def work_on_all_components(options) -> List[KernelComponentBase]:
  ...
  if options.sequential:
    return [kernel_component_factory(file) for file in ["vmlinux.o"] + files]
...
def main() -> int:
  ...
  def existing_file(file):
    if not os.path.isfile(file):
      raise argparse.ArgumentTypeError("{0} is not a valid file".format(file))
    return file
