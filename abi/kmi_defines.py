...
""" kmi_defines extract #define compile time constants from a Linux build.
    ...
    The only reason those comments were placed in an earlier version of the
    code was because the "Google Python Style Guide" strongly encourages to
    enable Python type analysis, see:
        https://google.github.io/styleguide/pyguide
    As a compromise, and to remove that clutter, all variables that don't have
    a type comment should be assumed to be of type str.  All other variables
    are left with their type, even if the type determination is obvious so as
    to ensure that ptype's checking is not affected.
"""
...
DEBUG = False  # type: bool # TODO: maybe turn into a command line argument
COMPILER = "clang"  # TODO: should be determined at run-time
...
def invalid_file(file: str) -> str:
    """ Return non-empty str if file doesn't exist or not a regular file. """
...
def makefile_assignment_left(assignment: str) -> str:
    ...
    left, _ = makefile_assignment_split(assignment)  # type: Tuple[str, str]
    return left
...
class Kmod:
    ...
    def get_kofile_name(self) -> str:
        ...
        """
        kofile_name, _ = os.path.splitext(self.base)  # type: Tuple[str, str]
        return kofile_name
...
class Info:
    ...
    def get_info_error(self) -> None:  # pylint: disable=no-self-use
        """ If self is an InfoError return self, return None otherwise.
...
class InfoError(Info):
    """ A failed attempt to create either and InfoKmod or an InfoKernel.
        ...
        When that creation fails an StopException is raised by stop(). The
        StopException contains it *args member the stop() message.
    """
...
def info_make(filename: str) -> Info:
    ...
    try:
        ...
        info = InfoError(filename, *stop_exception.args)  # type: Info
    return info


def info_show(info: Info):
    """ Show errors if info is an InfoError, print info if DEBUG is True. """
    info_error = info.get_info_error()  # type: InfoError
    if info_error:
        err = info_error.get_error()
        logging.error(info_error.filename + err)
        return
    if DEBUG:
        print("-" * 60, "{")
        info.print()
...
def work_on_whole_build() -> int:
    ...
    infos = work_on_all_kernel_components()  # type: List[Info]
...
def main() -> None:
    ...
    info = info_make(file)  # type: Info
    info_show(info)
    if info.get_info_error():  # type: InfoError
        return 1
    return 0
