...
class KernelModule:
    ...
    def __init__(self, kofile: str) -> None:
        ...
        left = makefile_assignment_left(self._cmd_text)
    ...
    def get_build_dir(self) -> str:
        ...
        if index > 0 and self._file[index - 1] == "/":
            index -= 1
...
class Kernel:
    ...
    def __init__(self, kernel: str) -> None:
        ...
        #         file = os.path.join(self._build_dir, file)
        #     self._archives_and_objects.append(file)
        self._archives_and_objects = [(os.path.join(self._build_dir, file)
                                       if path_is_relative(file) else file)
                                      for file in archives_and_objects]
    ...
    def __init__(self, filename: str) -> None:
        ...
        for obj in self._files_o:
            ...
            if path_is_relative(src):
                src = os.path.join(self._build_dir, self._source_dir)
...
def kernel_component_factory(filename: str) -> KernelComponentBase:
    ...
    # TODO(pantin): Matthias suggested "rather than only handling ExitError,
    # how about handling all sorts of Exceptions here?"
    #
    # I attempted that with this, but if I have code like:
    #   a = b
    # in the try above, that is use an undefined variable b, that is caught
    # too, the program instead of aborting for other language issues continues
    # running, which is not the intention at this stage for arbitrary
    # exceptions.  For StopError exceptions it is intended that they be caught
    # and transformed into a KernelComponentCreationError object.
    #
    # Unless other guidance is given, this comment and the code below will
    # go away shortly.
    #
    # except Exception as exception:
    #     return KernelComponentCreationError(filename, "Exception caught: " +
    #                                                   str(exception))
...
def all_ko_and_vmlinux() -> Iterable[str]:
    ...
    #   TODO(pantin): remove if no measuable slowdown when using (below):
    #
