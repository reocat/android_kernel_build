    ...
    for module, symbols in module_symbols.items():
        for symbol in symbols:
            if symbol not in exported:
                print(
                    "Symbol {} required by {} but not provided".format(
                        symbol, module
                    )
                )
...
def main():
    ...
    parser.add_argument(
        ...
        dest="report_missing",
        help=(
            "Do not report symbols required by modules, but missing from"
            " vmlinux"
        ),
    )
    ...
    if args.emit_module_symbol_lists and not args.symbol_list:
        print(
            "Emitting module symbol lists requires the --symbol-list parameter."
        )
        return 1
    ...
    if args.module_filters:
        modules = [
            ...
            if any(
                [
                    re.search(f, os.path.basename(mod))
                    for f in args.module_filters
                ]
            )
        ]
    ...
    if args.print_modules:
        if local_modules:
            print(
                "These modules have been considered when creating the symbol"
                " list:"
            )
            print(
                "  "
        ...
        if gki_modules:
            ...
            print(
                "  "
                + "\n  ".join(
                    sorted([os.path.basename(mod) for mod in gki_modules])
                )
            )
