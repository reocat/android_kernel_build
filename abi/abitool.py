...
class Stg(AbiTool):
    ...
    def diff_abi(self, old_dump, new_dump, diff_report, short_report=None,
        ...
        log.info(f'stgdiff {old_dump} {new_dump} at {prefix}.*')
        command = ['stgdiff', '--abi', old_dump, new_dump]
        for f in ['plain', 'flat', 'small', 'viz']:
            command.extend([f'--format {f}', f'--output {prefix}.{f}'])

        abi_changed = False

        with open(f'{prefix}.errors', 'w') as out:
            try:
                subprocess.check_call(command, stdout=out, stderr=out)
            except subprocess.CalledProcessError as e:
                if e.returncode & DIFF_ERROR:
                    raise
                abi_changed = True
...
def get_abi_tool(abi_tool = "libabigail"):
    if abi_tool == 'libabigail':
        ...
        return Libabigail()
    if abi_tool == 'stg':
        log.info('using stg for abi analysis')
        return Stg()
