...
class Combined(AbiTool):
    ...
    def diff_abi(self, old_dump, new_dump, diff_report, short_report=None,
        ...
        if abidiff_leaf_changed or abidiff_full_changed or stgdiff_changed:
            print("ABI DIFFERENCES HAVE BEEN DETECTED!")
            if abidiff_leaf_changed: print(" by abidiff (leaf-changes)")
            if abidiff_full_changed: print(" by abidiff (normal)")
            if stgdiff_changed: print(" by stgdiff")
            if print_report:
                with open(stg_small + ".short") as input:
                    for line in input:
                        print(line, end="")
            return True
        return False
