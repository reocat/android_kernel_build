...
def _collapse_CRC_changes(text, limit):
    ...
    def emit_pending():
        ...
        count = len(pending)
        if count > limit:
            new_lines.append("  [C] {} omitted; {} symbols in total have CRC changes only\n\n".format(count - limit, count))
        del pending[:]
    ...
    while index < len(lines):
        ...
        if index + 2 < len(lines):
            if change_regex.match(line) and crc_regex.match(lines[index+1]) and blank_regex.match(lines[index+2]):
