...
class RepoWrapper:
    ...
    def do_sync(self):
        ...
        if self.sync != "true":
            logging.info("`repo sync` is skipped.")
            manifest = self._superproject_root / ".repo/manifests" / _KLEAF_MANIFEST
            logging.info(
                "Generated manifest is at %s, included in default.xml", manifest)
            logging.info("** DON'T FORGET to commit the manifest change and "
                         "run `repo sync` !**")
            return
