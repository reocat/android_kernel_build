...
class KleafProjectSetter:
    ...
    def _load_meta_files(self):
        ...
        if self.prebuilts_dir:
            ...
            if self._can_download_artifacts():
                self._download_file_of_build(self.prebuilts_dir, "manifest.xml")
            self._repo_manifest_of_build = (
                self.prebuilts_dir / "manifest.xml").read_text()
            ...
            with tempfile.TemporaryDirectory() as meta_files_dir:
                ...
                self._download_file_of_build(pathlib.Path(meta_files_dir),
                                             "manifest.xml")
                self._repo_manifest_of_build = (
                    pathlib.Path(meta_files_dir) / "manifest.xml").read_text()
        else:
            self._download_list = {}
            self._repo_manifest_of_build = None
