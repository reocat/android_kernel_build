...
def _resolve(opt_path: pathlib.Path | None):
  if opt_path:
...
class KleafProjectSetter:
  ...
  def __init__(self, cmd_args: argparse.Namespace):
    self.ddk_workspace: pathlib.Path | None = _resolve(cmd_args.ddk_workspace)
    self.kleaf_repo_dir: pathlib.Path | None = _resolve(cmd_args.kleaf_repo_dir)
    self.prebuilts_dir: pathlib.Path | None = _resolve(cmd_args.prebuilts_dir)
  ...
  def _generate_module_bazel(self):
    ...
    with open(module_bazel, "w", encoding="utf-8") as f:
      ...
      if self.prebuilts_dir:
        f.write(_MODULE_BAZEL_PREBUILTS_CONTENT_TEMPLATE.format(
  ...
  def _add_submodules(self, git_root, kleaf_repo_dir, projects):
    ...
    if self.headers_hack:
      ...
      subprocess.run(["git", "submodule", "deinit", "-f",
        kleaf_repo_rel / "build/kernel"], cwd=git_root)
    ...
    if self.headers_hack:
      ...
      if not self._is_project_in_denylist(pathlib.Path(project_metadata["path"])):
        project_abs_path = kleaf_repo_dir / project_metadata["path"]
        subprocess.check_call([
          "git",
          "fetch",
          project_metadata["remote"]["fetch"] + project_metadata["name"],
          self.headers_hack], cwd=project_abs_path)
        subprocess.check_call([
  ...
  def _set_build_info(self):
    ...
    self.build_info = json.load(io.TextIOWrapper(build_info_fp, encoding="utf-8"))
  ...
  def _download_prebuilts(self):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
      ...
      for file in files:
        ...
        futures.append(executor.submit(self._download, file, open(dst, "wb"), close=True))
