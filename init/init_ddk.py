...
kernel_prebuilt_ext.declare_kernel_prebuilts(
    name = "gki_prebuilts",
    download_configs_file = "{download_configs}",
...
class KleafProjectSetter:
    ...
    def _generate_module_bazel(self):
        ...
        if self.prebuilts_dir:
            ...
            prebuilts_dir_rel = self._try_rel_workspace(self.prebuilts_dir)
            module_bazel_content += "\n"
            module_bazel_content += _LOCAL_PREBUILTS_CONTENT_TEMPLATE.format(
                download_configs=(prebuilts_dir_rel / "download_configs.json"),
                prebuilts_dir_relative=prebuilts_dir_rel,
            )
