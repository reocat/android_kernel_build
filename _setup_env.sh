...
# Example:
#   extract_git_meta_from_map "foo:123 bar:456" foo/baz
#   -> 123
function extract_git_meta_from_map() {
  local map=$1
...
export -f extract_git_meta_from_map

if [ -z "${SOURCE_DATE_EPOCH}" ]; then
  if [[ -n "${KLEAF_SOURCE_DATE_EPOCHS}" ]]; then
    export SOURCE_DATE_EPOCH=$(extract_git_meta_from_map "${KLEAF_SOURCE_DATE_EPOCHS}" "${KERNEL_DIR}" SOURCE_DATE_EPOCH)
