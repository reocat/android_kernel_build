...
# USERCFLAGS and USERLDFLAGS are set when --experimental_kernel_use_resolved_toolchains is set
# See prebuilts/clang/host/linux-x86/kleaf
...
if [ "${HERMETIC_TOOLCHAIN:-0}" -eq 1 ]; then
  ...
  # HOSTCFLAGS and HOSTLDFLAGS are set when --experimental_kernel_use_resolved_toolchains is set
  # See prebuilts/clang/host/linux-x86/kleaf
fi
