...
class OneProgress(object):
    ...
    def __str__(self):
        return f"{self.passed}/{self.expect_len}({self.passed / self.expect_len * 100:.2f}%)"
...
def print_progress():
    sys.stdout.write("; ".join([f"{k}: {v}" for k, v in PROGRESS.items()]))
    sys.stdout.write("\r")
    sys.stdout.flush()
...
def dump_chunk(src: io.IOBase, dst: io.IOBase, expect_len: int = None,
               name: str = None) -> bool:
    buf = src.read(4096)
    if not buf:
        return False
...
async def dump_file(src: io.IOBase, dst: io.IOBase, expect_len: int = None,
                    name: str = None) -> None:
    while True:
        if not dump_chunk(src, dst, expect_len, name):
            break
    ...
    with opener.open(req) as resp:
        ...
        with open(output_file, "wb") as f:
            ...
            try:
                ...
                traceback.print_exc()
            await dump_file(resp, f, expect_len=expect_len,
                            name=file)
...
async def download_artifact(build_number: str, target: str, file: str, out: str,
                            verbose: int = 0) -> None:
    ...
    await download_to(url, out, file, verbose=verbose, hook=signed_url_hook)
...
async def download_artifacts(build_number: str, target: str, files: list[str],
    ...
    tasks = [asyncio.create_task(download_artifact(
    ...
    if print_progress_task is not None:
