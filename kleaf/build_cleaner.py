...
Given a list of target patterns, the script:

- Finds all dependencies of the targets matching the given target patterns [Note 1]
- For all targets and their dependencies, fix BUILD files

Currently, the script supports fixing the following:

- `kernel_module.deps` [Note 2]
- `ddk_module.deps`    [Note 2]

Notes:
    1. https://bazel.build/run/build#specifying-build-targets
    2. This only works when all modules of the device are specified, e.g.
       by specifying the `dist` target which depends on all modules. See example
       below.
...
class DdkCleaner(object):
    def __init__(self, workspace_root: pathlib.Path, args: argparse.Namespace, color: bool):
    ...
    def _calc(self):
        ...
        kernel_module_target_strs: list[str] = subprocess.check_output(query_args,
...
class BuildCleaner(buildozer_command_builder.BuildozerCommandBuilder):
    def __init__(self, *init_args, **init_kwargs):
        ...
        self._ddk_cleaner = DdkCleaner(
            workspace_root=self._workspace_root(),
            args=self.args,
            color=(self.stdout.isatty() or self.stderr.isatty()),
        )
