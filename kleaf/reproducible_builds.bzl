...
SourceDateEpochInfo = provider(
    doc = "Provider of [`source_date_epoch()`](#source_date_epoch).",
    fields = {
...
def _source_date_epoch_impl(ctx):
    ...
    if (len(ctx.files.git_dir) == 0):
        ...
        infer_command = "echo '' > {out}".format(out = out.path)
    else:
        # Like _setup_env.sh, set SOURCE_DATE_EPOCH to empty string if the `git` command fails.
        # Suppress the exit code.
        infer_command = "git -C {git_dir} log -1 --pretty=%ct > {out} || true".format(
...
def source_date_epoch(name):
    ...
    See
    [explanations of `SOURCE_DATE_EPOCH`](https://reproducible-builds.org/docs/source-date-epoch/).
