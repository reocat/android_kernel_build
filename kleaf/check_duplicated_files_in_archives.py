...
def _list_files(archive: pathlib.Path) -> list[str]:
  with tarfile.open(archive) as tar:
    return [_sanitize(info.name) for info in tar.getmembers()]


def main(archives: Sequence[pathlib.Path]) -> None:
  """Check that when extracting each archive to the same directory, files won't
  be overwritten.
