...
"""Utility script to check if there are duplicated files in a given folder."""
...
def _list_files(archive: pathlib.Path) -> list[str]:
  if os.path.isfile(archive):
    ...
    return [
        _sanitize(os.path.relpath(os.path.join(root, file), archive))
...
def main(archives: Collection[pathlib.Path]) -> None:
  ...
  duplicated = {
      f: f_archives
      for f, f_archives in reverse_dict.items()
      if len(f_archives) > 1
  }
  if duplicated:
