...
class BazelWrapper(object):
    def __init__(self, root_dir: str, bazel_args_list: list[str], env):
        ...
        Args:
            ...
            bazel_args_list: The list of arguments the user provides through command line
        ...
        self.bazel_path = f"{self.root_dir}/prebuilts/bazel/linux-x86_64/bazel"
        self.absolute_out_dir = f"{self.root_dir}/out"

        command_idx = None
        for idx, arg in enumerate(bazel_args_list):
            if not arg.startswith("-"):
                command_idx = idx
                break

        self.startup_options, self.command, remaining_args = _partition(bazel_args_list,
                                                                        command_idx)
    ...
    def _build_final_args(self) -> list[str]:
        """Build the final arguments for the subprocess."""
        # final_args:
        # bazel [startup_options] [additional_startup_options] command [transformed_command_args] -- [target_patterns]

        bazel_jdk_path = f"{self.root_dir}/prebuilts/jdk/jdk11/linux-x86"
        ...
        final_args = [self.bazel_path] + self.startup_options + [
            ...
            f"--bazelrc={self.root_dir}/{bazelrc_name}",
        ]
...
if __name__ == "__main__":
    BazelWrapper(root_dir=sys.argv[1], bazel_args_list=sys.argv[2:], env=os.environ).run()
