...
class BazelWrapper(KleafHelpPrinter):
    ...
    def _get_workspace_dir(self):
        ...
        absolute_out_dir = known_startup_options.output_root
        transformed_startup_options = []
        # If --output_root or --output_user_root is specified, use the given
        # --output_user_root in `bazel info workspace`. Otherwise use the
        # default --output_user_root (in $HOME/.cache/bazel/_bazel_$USER).
        if absolute_out_dir:
            absolute_user_root = known_startup_options.output_user_root or \
                absolute_out_dir / "bazel/output_user_root"
            transformed_startup_options.append(f"--output_user_root={absolute_user_root}")
