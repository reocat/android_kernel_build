...
def kernel_build_strip_modules_test(name):
    ...
    for strip_modules in (True, False, None):
        ...
        _strip_modules_test(
            name = name + "_strip_modules_" + strip_modules_str + "_build_test",
            target_under_test = name + "_" + strip_modules_str + "_build",
            action_mnemonic = "KernelBuild",
            expect_strip_modules = bool(strip_modules),
        )
        tests.append(name + "_strip_modules_" + strip_modules_str + "_build_test")

        kernel_module(
            name = name + "_" + strip_modules_str + "_module",
            kernel_build = name + "_" + strip_modules_str + "_build",
            tags = ["manual"],
        )
        _strip_modules_test(
            name = name + "_strip_modules_" + strip_modules_str + "_module_test",
            target_under_test = name + "_" + strip_modules_str + "_module",
            action_mnemonic = "KernelModule",
            expect_strip_modules = strip_modules,
        )
        tests.append(name + "_strip_modules_" + strip_modules_str + "_module_test")

        kernel_build_abi(
            name = name + "_" + strip_modules_str + "_abi",
            build_config = "build.config.fake",
            outs = [],
            define_abi_targets = True,
            strip_modules = strip_modules,
            # TODO: Make mixed build work as well.
            # base_kernel = name + "_" + strip_modules_str + "_build",
            tags = ["manual"],
        )
        _strip_modules_test(
            name = name + "_strip_modules_" + strip_modules_str + "_abi_test",
            target_under_test = name + "_" + strip_modules_str + "_abi",
            action_mnemonic = "KernelBuild",
            expect_strip_modules = strip_modules,
        )
        tests.append(name + "_strip_modules_" + strip_modules_str + "_abi_test")
