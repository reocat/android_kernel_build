...
# TODO(b/233247999): flip this when Bazel is updated
EXTRA_ASPECTS_SUPPORTED = False

def _kbuild_symtypes_test_make_vars(ctx, env):
    if not EXTRA_ASPECTS_SUPPORTED:
        return
...
def _make_kbuild_symtypes_test(kbuild_symtypes_flag_value):
    extra_kwargs = {}
    if EXTRA_ASPECTS_SUPPORTED:
        extra_kwargs["extra_target_under_test_aspects"] = [kernel_env_aspect]
