...
class ScmVersionTestCase(unittest.TestCase):
  _scmversion_pattern = re.compile(r"^Linux version [0-9]+[.][0-9]+[.][0-9]+(-[0-9]+)?-g[0-9a-f]{12}")
  def test_vmlinux_contains_scmversion(self):
    ...
    for artifact in arguments.artifacts:
      if os.path.basename(artifact) == "vmlinux":
        strings = subprocess.check_output(["strings", artifact], text = True).strip().splitlines()
        self.assertTrue(any(re.search(ScmVersionTestCase._scmversion_pattern, s) for s in strings),
