...
class KleafIntegrationTest(unittest.TestCase):
  ...
  def _check_call(
      self, command: str, command_args: list[str], startup_options=(), **kwargs
  ) -> None:
  ...
  def test_module_does_not_depend_on_vmlinux(self):
    ...
    print(
        '+ build/kernel/kleaf/analysis/inputs.py \'mnemonic("KernelModule.*",'
        f" {vd_modules[0]})'"
    )
  ...
  def test_change_lto_to_thin_when_local(self):
    ...
    self._build([f"//{self._common()}:kernel_dist"] + _LOCAL + ["--lto=thin"])
  ...
  def test_change_lto_to_none_when_local(self):
    ...
    """
    self._check_call(
        "build", [f"//{self._common()}:kernel_dist"] + _LOCAL + ["--lto=thin"]
    )
    self._check_call(
  ...
  def test_config_uapi_header_test(self):
    ...
    self.assertTrue(
        "CONFIG_UAPI_HEADER_TEST=y" in new_gki_defconfig_content.splitlines(),
        (
            "gki_defconfig should still have CONFIG_UAPI_HEADER_TEST=y after"
