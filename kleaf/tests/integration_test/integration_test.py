...
class DdkWorkspaceSetupTest(KleafIntegrationTestBase):
    ...
    def test_kleaf_module_below_ddk_workspace(self):
        ...
        if not arguments.mount_spec:
            ...
            self._unshare_mount_run(mount_spec=mount_spec, link_spec=[])
            return
    ...
    def test_setup_with_prebuilts(self):
        ...
        if not arguments.mount_spec:
            ...
            ddk_projects = [project for project in projects
                            if "ddk" in project.groups or
                               "ddk-external" in project.groups]

            mount_spec = {}
            link_spec = []
            # Mimic init_ddk's behavior that, when build_id is set, checkout
            # projects & create symlinks.
            for project in ddk_projects:
