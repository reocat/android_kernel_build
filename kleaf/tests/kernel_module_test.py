...
class ScmVersionTestCase(unittest.TestCase):
  ...
  def _assert_contains_scmversion(self, module):
    strings = subprocess.check_output(["strings", module], text = True).strip().splitlines()
    self.assertTrue(any(re.match(ScmVersionTestCase._scmversion_pattern, s) for s in strings),
