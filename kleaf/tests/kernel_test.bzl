...
def _kernel_module_test_impl(ctx):
    ...
    arguments_file = _write_script_and_arguments(ctx, arguments)
    return [DefaultInfo(runfiles = ctx.runfiles(files = ctx.files.modules + [arguments_file]))]
...
def _kernel_build_test_impl(ctx):
    ...
    arguments_file = _write_script_and_arguments(ctx, arguments)
    return [DefaultInfo(runfiles = ctx.runfiles(files = ctx.files.target + [arguments_file]))]
