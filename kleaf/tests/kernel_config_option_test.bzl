...
def _combined_test_actual_impl(ctx):
    ...
    for lto, kernel_build in ctx.split_attr.kernel_build.items():
        # Test LTO setting
        files.append(_symlink_config(ctx, kernel_build, paths.join(lto, lto + "_config")))

        # Test trim setting
        files.append(_symlink_config(ctx, kernel_build, paths.join(lto, ctx.attr.prefix + "_config")))
...
_combined_test_actual = rule(
    ...
    attrs = dicts.add(_get_config_attrs_common(lto_transition), {
        "prefix": attr.string(),
    }),
)
...
def _combined_option_test(name, kernels):
    ...
    for trim in (True, False):
        ...
        native.filegroup(
            ...
            srcs = native.glob(["data/kernel_config_option_test/*_config"]),
        )
        contain_lines_test(
            name = test_name,
