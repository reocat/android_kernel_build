...
class CompareTest(unittest.TestCase):
    def test_all(self):
        ...
        for path in arguments.actual:
            actual[os.path.basename(path)].append(path)

        expected = collections.defaultdict(list)
        for path in arguments.expected:
            expected[os.path.basename(path)].append(path)
        ...
        for basename in basenames:
            ...
            self.assertIsNotNone(actual_with_basename, f"missing actual file for {basename}")
            self.assertIsNotNone(expected_with_basename, f"missing expected file for {basename}")
    ...
    def _expect_contain_lines(self, actual, expected):
