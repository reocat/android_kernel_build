...
def _kgdb_test(name, kernels):
    """Test the effect of a `--kgdb` on `kernel_config`."""

    tests = []
    for arch, kernel_build in kernels.items():
        _transition_test(
            name = name + "_" + arch,
            kernel_build = kernel_build,
...
def kernel_config_option_test_suite(name):
    ...
    _lto_test(
        name = name + "_lto_test",
        kernel_build = name + "_kernel_aarch64",
    )
    ...
    _kgdb_test(
        ...
        kernels = {
            "aarch64": name + "_kernel_aarch64",
            "x86_64": name + "_kernel_x86_64",
        },
    )
