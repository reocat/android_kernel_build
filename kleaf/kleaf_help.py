...
_FLAG_PATTERN = re.compile(
    r"build"
    r" --flag_alias=(?P<short_name>[a-z_]+)=(?P<is_negated>no)?(?P<label>[a-zA-Z/:_-]+)$"
)
...
class KleafHelpPrinter(object):
  ...
  def print_kleaf_help(self, kleaf_repo_dir: pathlib.Path):
    ...
    parser = argparse.ArgumentParser(
        ...
        add_help=False,
        usage=(
            "bazel [<startup options>] <command> [<args>] [--] [<target"
            " patterns>]"
        ),
    ...
    bazelrc_parser = FlagsBazelrcParser(kleaf_repo_dir / FLAGS_BAZEL_RC)
    bazelrc_parser.add_to(parser, kleaf_repo_dir=kleaf_repo_dir)

    config_group = parser.add_argument_group(title="Args - configs")
    for f in os.listdir(kleaf_repo_dir / _BAZEL_RC_DIR):
      bazelrc_parser = ConfigBazelrcParser(kleaf_repo_dir / _BAZEL_RC_DIR / f)
      bazelrc_parser.add_to(config_group, kleaf_repo_dir=kleaf_repo_dir)
    ...
    parser.add_argument_group(
        title="Target patterns", description="$ bazel help target-syntax"
    )
...
class FlagAlias(object):
  ...
  def add_to_group(self, group, kleaf_repo_dir: pathlib.Path):
    ...
    group.add_argument(f"--{self.short_name}", **kwargs)
...
class FlagsSection(BazelrcSection):
  ...
  def add_to(
      self, parser: argparse.ArgumentParser, kleaf_repo_dir: pathlib.Path
  ):
    if not self.flags:
...
class ConfigSection(BazelrcSection):
  ...
  def add_to(self, group, kleaf_repo_dir: pathlib.Path):
    ...
    description = "\n".join(description)
    group.add_argument(
        f"--config={config}", help=description, action="store_true"
    )
