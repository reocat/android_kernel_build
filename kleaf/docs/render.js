...
function setStyle(nodes, clz) {
    for (var i = 0; i < nodes.length; i++) {
        nodes[i].classList.add(clz);
    }
}

function refreshStyle(node) {
    for (var i = 0; i <= 6; i++) {
        setStyle(node.getElementsByTagName("h" + i), "mdc-typography--headline" + i)
    }
}
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    get(new URL("directory.html.frag", document.location).href, function(request) {
        ...
        refreshStyle(document.getElementById("div-directory-frag"));
    }, function(request) {
