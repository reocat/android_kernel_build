...
Executing the command with `--commit` creates a git commit with
...
The command executes `git commit --amend` at the end, which brings up your
pre-configured text editor for git to edit the commit message. You may
edit the subject line, add additional message, and add a bug number.
