...
When updating ABI symbol list xml file for the first time or a branch has no
existing symbol list file; this error will be generated when running the ABI
symbol list update target. This happens as tool can not find a file to update,
in that case macro expander doesn't generate this target.

To work around this; first create an empty symbol list xml file using:

```shell
touch common/android/abi_gki_aarch64.xml
```

Second, run symbol list update with no diff as reference file will be an empty file using:

```shell
bazel run //common:kernel_aarch64_abi_nodiff_update
```

This will geneate the base line symbol list xml file and the symbol list update target
`bazel run //common:kernel_aarch64_abi_update` will be available now onwards.
