...
without a SHA, the SCM version is not determined correctly.

The SCM version is disabled for `--config=skip_scm` or any other configs that
inherits from it (e.g. `--config=local` or `--config=fast`).

The SCM version should be embedded properly on release builds, where
`--config=skip_scm` should not be specified.
