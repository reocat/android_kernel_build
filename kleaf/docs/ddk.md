...
build has been done. Given the kernel build can be compiled with Kleaf today,
the Kleaf team proposes we compile module builds with Kleaf, using the GKI
kernel build as an input. This model has been implemented for the Pixel 2021
kernel, wrapping the module build invocations in the `kernel_module()`
...
The Kleaf team proposes to entirely define the **module build with Bazel
(starlark) macros**
