...
def _kernel_module_impl(ctx):
    ...
    _debug_print_scripts(ctx, command, what = "kernel_uapi_headers_archive")
    ctx.actions.run_shell(
        mnemonic = "KernelModuleUapiHeadersArchive",
        inputs = ctx.attr._hermetic_tools[HermeticToolsInfo].deps + [kernel_uapi_headers_dir],
        outputs = [kernel_uapi_headers_archive],
        command = command,
        progress_message = "Creating kernel_uapi_headers_archive {}".format(ctx.label),
    )
