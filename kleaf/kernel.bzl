...
def kernel_build(
    ...
    Args:
        ...
        outs: the expected output files. For each item {out}, the build rule
            automatically finds a matching file in the legacy kernel build
            output directory ${OUT_DIR}, using the following command:
              find ${OUT_DIR} -wholename */{out}
            There must be exactly one match.
            The file is copied to the following in the output directory:
              1. {name}/{out}
              2. If {out} contains a slash, {name}/$(basename {out})
            ...
            The bulid system copies
              ${OUT_DIR}/[<optional subdirectory>/]arch/arm64/boot/vmlinux
            to:
            ...
            See search_and_copy_output.sh for details.
...
def _kernel_build(
    ...
    kwargs["tools"] += _kernel_build_tools(env_target_name, toolchain_version) + [
        "//build/kleaf:search_and_copy_output.sh",
    ]
    ...
    for out in outs:
        ...
        cmd = """
              $(execpath //build/kleaf:search_and_copy_output.sh) "{name}" "{out}" "$${{OUT_DIR}}" "$(@D)/{name}"
    ...
    native.genrule(
        ...
        cmd = _kernel_build_common_setup(env_target_name) +
            ...
            # make -C $${{KERNEL_DIR}} $${{TOOL_ARGS}} O=$${{OUT_DIR}} $${{MAKE_GOALS}}
              mkdir -p $${{OUT_DIR}}/arch/arm64/boot $${{OUT_DIR}}/barbaz/arch/arm64/boot $${{OUT_DIR}}/foobar
              touch $${{OUT_DIR}}/vmlinux
              """.format(name = name) +
              ("".join(out_cmd)),
