...
def kernel_build_abi(
    ...
    kernel_build_abi_dist(name = "kernel_aarch64_dist", data = _dist_targets)
    kernel_build_abi_dist(name = "kernel_aarch64_abi_dist", data = _dist_targets + ["kernel_aarch64_abi"])
    ```
    ...
    - `kernel_aarch64_abi`
      - If `abi_definition` is not `None`, building this target compares the ABI with
        `abi_definition`.
      - Include this target in a [`kernel_build_abi_dist`](#kernel_build_abi_dist)
        target to copy ABI dump and diff report to `--dist-dir`.
...
def kernel_build_abi_dist(
    ...
    execrule(
        ...
        data = [
            ...
            # FIXME DON'T RELY ON NAMES
            kernel_build_abi + "_diff_executable",
        ],
        script = """
          # Copy to dist dir
        ...
        """.format(
            ...
            # FIXME DON'T RELY ON NAMES
            diff = kernel_build_abi + "_diff_executable",
        ),
    )
