...
def _env(env_target_name, build_config, build_config_srcs, **kwargs):
    ...
    Args:
        env_target_name: the name of the main build config
        build_config: the path to the build config from the directory containing
           the WORKSPACE file, e.g. "common/build.config.gki.aarch64"
...
def _config_rule_output(target_name):
    return [
        target_name + "/.config",
        target_name + "/include.tar.gz",
    ]
...
def _config(
    ...
    Args:
        config_target_name: the name of the kernel config
    ...
    native.genrule(
        ...
        cmd = _kernel_build_common_setup(env_target_name) + """
            ...
            # Grab outputs
              mv $${{OUT_DIR}}/.config $(location {config_target_name}/.config)
              tar czf $(location {config_target_name}/include.tar.gz) -C $${{OUT_DIR}} include/
            """.format(config_target_name = config_target_name),
...
def _kernel_build(
    ...
    # Rule that copies e.g. kernel_aarch64_config/.config to kernel_aarch64/.config
    native.genrule(
        name = name + "_copy_config_rule",
        srcs = [config_target_name + "/.config"],
        outs = [name + "/.config"],
        cmd = "cp $< $@",
    )
