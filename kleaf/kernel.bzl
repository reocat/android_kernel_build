...
def _boot_images_impl(ctx):
    ...
    if not ctx.attr.build_vendor_boot_image:
...
Execute `build_boot_images` in `build_utils.sh`.""",
    attrs = {
        ...
        "build_vendor_boot_image": attr.bool(),
        "_debug_print_scripts": attr.label(
            default = "//build/kernel/kleaf:debug_print_scripts",
        ),
        "_search_and_mv_output": attr.label(
...
def kernel_images(
        ...
        build_boot_image = None,
        build_vendor_boot_image = None,
    ...
    Args:
        ...
        kernel_build: A `kernel_build` rule. Must specify if `build_boot_image`.
        mkbootimg: Path to the mkbootimg.py script which builds boot.img.
          Keep in sync with `MKBOOTIMG_PATH`. Only used if `build_boot_image`. If `None`,
        ...
        boot_image_outs: A list of output files that will be installed to `DIST_DIR` when
          ...
          `build_vendor_boot_image = True`.

          If `build_boot_image` is equal to `False`, the default is empty.

          If `build_boot_image` is equal to `True`, the default list assumes the following:
          - `BOOT_IMAGE_FILENAME` is not set (which takes default value `boot.img`), or is set to
            `"boot.img"`
          - `vendor_boot.img` if `build_vendor_boot_image`
        ...
        build_boot_images: Deprecated. Use `build_boot_image` and `build_vendor_boot_image` instead.

          If set explicitly, it is equivalent to setting `build_boot_image = build_boot_images` and
          `build_vendor_boot_image = build_boot_images`.
        build_boot_image: Whether to build boot image. It must be set if either `BUILD_BOOT_IMG`
        ...
        build_vendor_boot_image: Whether to build `vendor_boot` image. It must be set if either
          `BUILD_BOOT_IMG` or `BUILD_VENDOR_BOOT_IMG` is set, and `SKIP_VENDOR_BOOT` is not set.

          If `True`, requires `build_boot_image = True`.
    ...
    if build_boot_images != None:
        if build_boot_image != None or build_vendor_boot_image != None:
            fail("{}: build_boot_images is deprecated. It cannot be set when build_boot_image and/or build_vendor_boot_image are set.".format(name))
        build_boot_image = build_boot_images
        build_vendor_boot_image = build_boot_images

    if build_vendor_boot_image and not build_boot_image:
        fail("{}: build_vendor_boot_image = True requires build_boot_image = True.".format(name))

    if build_boot_image:
        if build_initramfs == None:
            build_initramfs = True
        if not build_initramfs:
            fail("{}: Must set build_initramfs to True if build_boot_image".format(name))
        if kernel_build == None:
            fail("{}: Must set kernel_build if build_boot_image".format(name))

    # Set default value for boot_image_outs according to build_boot_image
    if boot_image_outs == None:
        if not build_boot_image:
            boot_image_outs = []
        else:
            boot_image_outs = [
                "boot.img",
    ...
    if build_vendor_boot_image and "vendor_boot.img" not in boot_image_outs:
    ...
    if build_boot_image:
        _boot_images(
            ...
            mkbootimg = mkbootimg,
            build_vendor_boot_image = build_vendor_boot_image,
        )
