...
def _kernel_build_impl(ctx):
    ...
    for attr in out_attrs:
        ...
        for out in getattr(ctx.outputs, attr):
            ...
            all_outs[attr].append(short_name)
    ...
    command = ctx.attr.config[_KernelEnvInfo].setup + """
        ...
        outdir = outdir.path,
        outs = " ".join(all_outs["outs"]),
        implicit_outs = " ".join(all_outs["implicit_outs"]),
        module_outs = " ".join(all_outs["module_outs"]),
    ...
    env_info_dependencies = ctx.attr.config[_KernelEnvInfo].dependencies + \
                            ctx.outputs.outs + ctx.outputs.implicit_outs
