...
def _kernel_env_impl(ctx):
    ...
    ctx.actions.run_shell(
        inputs = ctx.files.srcs + dependencies + [
            setup_env,
            preserve_env,
        ],
        outputs = [out_file],
...
def _kernel_build_impl(ctx):
    ...
    module_staging_dir = ctx.actions.declare_directory("{name}/staging".format(name = ctx.label.name))
    module_staging_archive = ctx.actions.declare_file("{name}/module_staging_dir.tar.gz".format(name = ctx.label.name))
