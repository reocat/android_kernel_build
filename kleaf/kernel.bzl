...
def _combine_dict(*args):
    """Combine dictionaries specified in args."""
    ret = {}
    for d in args:
        ret.update(d)
    return ret

def _get_hermetic_path_cmd(ctx):
    # This rule doesn't depend on a kernel_env target. Hence, we need to manually
    # set PATH to ensure we don't use tools outside of the sandbox.
    return """
        export PATH=$({readlink} -m {host_tools_path}):$({readlink} -m {kernel_build_tools_path})
    """.format(
        readlink = ctx.file._readlink.path,
        host_tools_path = ctx.files._host_tools[0].dirname,
        kernel_build_tools_path = ctx.files._kernel_build_tools[0].dirname,
    )

def _get_hermetic_path_deps(ctx):
    return ctx.files._host_tools + ctx.files._kernel_build_tools

_get_hermetic_path_dep_attrs = {
    "_readlink": attr.label(default = "//build/kernel:build-tools/path/linux-x86/readlink", allow_single_file = True),
    "_host_tools": attr.label(default = "//build/kernel:host-tools"),
    "_kernel_build_tools": attr.label(default = "//build/kernel:kernel-build-tools"),
}
...
def _merged_kernel_uapi_headers_impl(ctx):
    ...
    inputs = _get_hermetic_path_deps(ctx) + srcs
