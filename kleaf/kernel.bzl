...
def _kernel_kythe_impl(ctx):
    ...
    command += """
             # Generate compile_commands.json
               interceptor_analysis -l {interceptor_output} -o {compile_commands} -t compdb
    """.format(
        interceptor_output = interceptor_output.path,
    ...
    command += """
             ...
             # Generate kzips
               runextractor compdb -extractor $(which cxx_extractor) 2> {runextractor_error}

             # Package it all into a single .kzip, ignoring duplicates.
               for zip in $(find "$KYTHE_OUTPUT_DIRECTORY" -name '*.kzip'); do
                   unzip -qn "${{zip}}" -d {extracted_kzip_dir}
               done
               (cd {extracted_kzip_dir} && zip -9rmq {all_kzip} .)
