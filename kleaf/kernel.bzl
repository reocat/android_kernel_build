...
_KernelBuildExtModuleInfo = provider(
    ...
    fields = {
        ...
        "unstripped_modules": "Whether an external [`kernel_module`](#kernel_module) building against this [`kernel_build`](#kernel_build) should provide unstripped ones for debugging.",
    },
)
...
_KernelUnstrippedModulesInfo = provider(
    ...
    fields = {
        ...
        "unstripped_dir": """A [`File`](https://bazel.build/rules/lib/File) that
...
- This is `None` if and only if `unstripped_modules = False`
- Never `None` if and only if `unstripped_modules = True`
- An empty directory if and only if `unstripped_modules = True` and `module_outs` is empty

For an external [`kernel_module()`](#kernel_module), this is a directory containing unstripped external modules.
- This is `None` if and only if the `kernel_build` argument has `unstripped_modules = False`
- Never `None` if and only if the `kernel_build` argument has `unstripped_modules = True`
""",
    },
)
...
def _kernel_build_impl(ctx):
    ...
    kernel_build_module_info = _KernelBuildExtModuleInfo(
        ...
        unstripped_modules = ctx.attr.unstripped_modules,
    )
...
_kernel_build = rule(
    ...
    attrs = {
        ...
        "additional_kmi_symbol_lists_src": attr.label_list(allow_files = True, doc = "The **source** `additional_kmi_symbol_lists`"),
...
def _kernel_module_impl(ctx):
    ...
    unstripped_dir = None
    if ctx.attr.kernel_build[_KernelBuildExtModuleInfo].unstripped_modules:
        unstripped_dir = ctx.actions.declare_directory("{name}/unstripped".format(name = ctx.label.name))
...
def _kernel_extracted_symbols_impl(ctx):
    ...
    vmlinux = _find_file(name = "vmlinux", files = ctx.files.kernel_build_notrim, what = "{}: kernel_build_notrim".format(ctx.attr.name), required = True)
    in_tree_modules = _find_files(suffix = ".ko", files = ctx.files.kernel_build_notrim, what = "{}: kernel_build_notrim".format(ctx.attr.name))
...
def _kernel_update_symbols_dest_symlink_impl(ctx):
    # TODO(b/197938817): build_abi.sh supports missing kmi_symbol_list. Also support here.
    if not ctx.attr.kernel_build[_KernelBuildAbiInfo].kmi_symbol_list_src:
    ...
    ctx.actions.symlink(
        ...
        target_file = ctx.attr.kernel_build[_KernelBuildAbiInfo].kmi_symbol_list_src,
        progress_message = "Determining the destination for updating kmi_symbol_list {}".format(ctx.label),
    )

_kernel_update_symbols_dest_symlink = rule(
    implementation = _kernel_update_symbols_dest_symlink_impl,
    doc = "Create a symlink to the source `kmi_symbol_list` of the `kernel_build`, so `update_symbols.py` works.",
    attrs = {
        "kernel_build": attr.label(providers = [_KernelBuildAbiInfo]),
        "out": attr.output(doc = "Symlink to the source `kmi_symbol_list` of the `kernel_build`"),
    },
)
...
def _kernel_abi_dump_impl(ctx):
    ...
    return DefaultInfo(files = depset([full_abi_out_file, abi_out_file]))
...
def _kernel_abi_dump_full(ctx):
    ...
    for prov in unstripped_dir_providers:
        if not prov.unstripped_dir:
            fail("{}: Requires dep {} to set unstripped_modules = True".format(ctx.label, prov.label))
    unstripped_dirs = [prov.unstripped_dir for prov in unstripped_dir_providers]
    ...
    ctx.actions.run_shell(
        ...
        progress_message = "Creating ABI dump {}".format(ctx.label),
    )
...
def kernel_build_and_abi(
    ...
    |-------------------------------------------------------|-----------------------------------|
    |`bazel run kernel_aarch64_abi_update_symbol_list`      |`build_abi.sh --update_symbol_list`|
    ...
    _kernel_update_symbols_dest_symlink(
        name = name + "_abi_update_symbols_dest_symlink",
        kernel_build = name,
        out = name + "_abi_internal/update_destination",
    )

    copy_file(
        name = name + "_abi_tool",
        src = "//build/kernel/kleaf:update_symbols.py",
        out = name + "_abi_internal/dist.py",
    )

    native.py_binary(
        name = name + "_abi_update_symbol_list",
        srcs = [name + "_abi_internal/dist.py"],
        main = name + "_abi_internal/dist.py",
        python_version = "PY3",
        data = [
...
def _kernel_unstripped_modules_archive_impl(ctx):
    kernel_build = ctx.attr.kernel_build
    base_kernel = kernel_build[_KernelUnstrippedModulesInfo].base_kernel

    # Early elements = higher priority
    srcs = []
    if base_kernel:
        srcs.append(base_kernel[_KernelUnstrippedModulesInfo].unstripped_dir)
    srcs.append(kernel_build[_KernelUnstrippedModulesInfo].unstripped_dir)
    for kernel_module in ctx.attr.kernel_modules:
        srcs.append(kernel_module[_KernelUnstrippedModulesInfo].unstripped_dir)
    ...
    for src in reversed(srcs):
        ...
        command += """
            find {src} -name '*.ko' -exec cp {{}} {unstripped_dir} \\;
        """.format(
            src = src.path,
            unstripped_dir = unstripped_dir,
        )
    ...
    command += """
        tar -czf {out_file} -C $(dirname {unstripped_dir}) $(basename {unstripped_dir})
    """.format(
        out_file = out_file.path,
        unstripped_dir = unstripped_dir,
    )
...
It requires `unstripped_modules = True`. If the `kernel_build` has a `base_kernel`, the rule
also retrieves unstripped in-tree modules from the `base_kernel`, and requires the
`base_kernel` has `unstripped_modules = True`.
""",
...
It requires that the base `kernel_build` has `unstripped_modules = True`.
""",
            providers = [_KernelUnstrippedModulesInfo],
        ),
