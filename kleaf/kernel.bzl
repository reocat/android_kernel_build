...
def _kernel_build_impl(ctx):
    ...
    kernel_build_module_info = _ExtModuleInfo(
...
def _kernel_module_impl(ctx):
    ...
    modules_prepare = ctx.attr.kernel_build[_ExtModuleInfo].modules_prepare
    inputs = []
    inputs += ctx.files.srcs
    inputs += ctx.attr.kernel_build[_KernelEnvInfo].dependencies
    inputs += modules_prepare[_KernelEnvInfo].dependencies
    inputs += ctx.attr.kernel_build[_ExtModuleInfo].module_srcs
...
""",
    attrs = {
        ...
        "kernel_build": attr.label(
            mandatory = True,
            providers = [_KernelEnvInfo, _ExtModuleInfo],
        ),
...
def _kernel_modules_install_impl(ctx):
    ...
    inputs += [
        ...
        ctx.attr.kernel_build[_ExtModuleInfo].modules_staging_archive,
    ]
    ...
    """.format(
        ...
        kernel_build_modules_staging_archive =
            ctx.attr.kernel_build[_ExtModuleInfo].modules_staging_archive.path,
    )
    for kernel_module in ctx.attr.kernel_modules:
