...
def _kmi_symbol_list_strict_mode(ctx, all_output_files, all_module_names_file):
    ...
    command = ctx.attr.config[_KernelEnvInfo].setup + """
        KMI_STRICT_MODE_OBJECTS="{vmlinux_base} $(cat {all_module_names_file})" {compare_to_symbol_list} {module_symvers} {raw_kmi_symbol_list}
        touch {out}
    """.format(
...
def _kernel_build_impl(ctx):
    ...
    command += """
         ...
         # Check if there are remaining *.ko files
           remaining_ko_files=$(comm -13 <(cat {all_module_names_file} | sort) <(find {modules_staging_dir}/lib/modules/*/kernel -type f -name '*.ko' -exec basename {{}} \\; | sort))
