...
def kernel_build(
    ...
    kmi_symbol_list_target_name = name + "_combined_kmi_symbol_list"
...
def _kernel_extracted_symbols_impl(ctx):
    ...
    in_tree_modules = _find_files(suffix = ".ko", files = ctx.files.kernel_build, what = "{}: kernel_build".format(ctx.attr.name))
    ...
    if ctx.attr.kernel_build[_KernelBuildAbiInfo].trim_nonlisted_kmi:
...
def kernel_abi(
    ...
    Args:
      name: Name of target.
      kernel_build: The `kernel_build` that provides vmlinux, in-tree modules
        and toolchain. This should be a `kernel_build` with
        `trim_nonlisted_kmi = False`.
