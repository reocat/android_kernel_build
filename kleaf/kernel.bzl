...
_KernelModuleDevInfo = provider(
    doc = "A provider that specifies the expectations of a `_kernel_module` or a `kernel_modules_install` from its `kernel_build` attribute.",
    fields = {
...
def _kernel_build_impl(ctx):
    ...
    kernel_build_module_info = _KernelModuleDevInfo(
...
def _kernel_module_impl(ctx):
    ...
    modules_prepare = ctx.attr.kernel_build[_KernelModuleDevInfo].modules_prepare
    inputs = []
    inputs += ctx.files.srcs
    inputs += ctx.attr.kernel_build[_KernelEnvInfo].dependencies
    inputs += modules_prepare[_KernelEnvInfo].dependencies
    inputs += ctx.attr.kernel_build[_KernelModuleDevInfo].module_srcs
...
""",
    attrs = {
        ...
        "kernel_build": attr.label(
            mandatory = True,
            providers = [_KernelEnvInfo, _KernelModuleDevInfo],
        ),
...
def _kernel_modules_install_impl(ctx):
    ...
    inputs += [
        ...
        ctx.attr.kernel_build[_KernelModuleDevInfo].modules_staging_archive,
    ]
    ...
    """.format(
        ...
        kernel_build_modules_staging_archive =
            ctx.attr.kernel_build[_KernelModuleDevInfo].modules_staging_archive.path,
    )
    for kernel_module in ctx.attr.kernel_modules:
...
def _kernel_filegroup_impl(ctx):
    ...
    kernel_module_dev_info = _KernelModuleDevInfo(
        ...
        # TODO(b/218761417): implement _KernelEnvInfo for the modules_prepare target
    ...
    return [
        ...
        # TODO(b/218761417): implement _KernelEnvInfo for _kernel_build
    ]
