...
def _kernel_module_impl(ctx):
        ...
        if kernel_module_dep[KernelModuleInfo].kernel_build != \
           ctx.attr.kernel_build:
            fail((
                ...
                dep = kernel_module_dep,
                dep_kernel_build = kernel_module_dep[KernelModuleInfo].kernel_build,
            ))
    ...
    command += """
             ...
             # Archive module_staging_dir
               (cd {module_staging_dir} && tar czf {module_staging_archive} lib/modules/*/extra/{{{comma_separated_outs}}})
