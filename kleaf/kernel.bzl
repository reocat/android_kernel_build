...
def _merged_kernel_uapi_headers_impl(ctx):
    ...
    inputs = ctx.files._host_tools + ctx.files._build_tools + srcs
    ...
    command = """
      ...
      # set PATH to ensure we don't use tools outside of the sandbox.
        export PATH={host_tools_base}:{build_tools_base}
        mkdir -p {intermediates_dir}
    """.format(
        host_tools_base = ctx.files._host_tools[0].dirname,
        build_tools_base = ctx.files._build_tools[0].dirname,
...
""",
    attrs = {
        ...
        #        "_tar": attr.label(default = "//build/kernel:host-tools/tar", allow_single_file = True),
        #        "_gzip": attr.label(default = "//build/kernel:build-tools/path/linux-x86/gzip", allow_single_file = True),
        #        "_mkdir": attr.label(default = "//build/kernel:build-tools/path/linux-x86/mkdir", allow_single_file = True),
        #        "_rm": attr.label(default = "//build/kernel:build-tools/path/linux-x86/rm", allow_single_file = True),
        "_host_tools": attr.label(default = "//build/kernel:host-tools"),
        "_build_tools": attr.label(default = "//build/kernel:kernel-build-tools"),
        "_debug_print_scripts": attr.label(default = "//build/kernel/kleaf:debug_print_scripts"),
    },
)
