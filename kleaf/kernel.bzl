...
def kernel_build(
    ...
    Args:
        ...
        kmi_symbol_lists: A list of labels referring to the KMI symbol list files.
          ...
          If non-empty, `abi_symbollist` and `abi_symbollist.report` are created
          and added to the
          [`DefaultInfo`](https://docs.bazel.build/versions/main/skylark/lib/DefaultInfo.html),
          ...
          This may be a `glob()` to determine whether `abi_symbollist`
...
def _kernel_config_impl(ctx):
    ...
    if ctx.file.raw_kmi_symbol_list:
        ...
        #   the _kernel_config rule, which is destroyed during the execution of
        #   _kernel_build.
    ...
    if ctx.file.raw_kmi_symbol_list:
        ...
        # interpreted as a path relative to ${ROOT_DIR}/${KERNEL_DIR}
...
_kernel_config = rule(
    ...
    attrs = {
        ...
        "raw_kmi_symbol_list": attr.label(
            doc = "Label to abi_symbollist.raw. If specified, modify the config to trim non-listed KMI.",
            allow_single_file = True,
        ),
        "_debug_print_scripts": attr.label(default = "//build/kleaf:debug_print_scripts"),
    },
