...
def _kernel_build_impl(ctx):
    ...
    # kernel_build(name="kenrel", outs=["foo"])
    # => _kernel_build(name="kernel", outs=["kernel/foo"])
    # => all_outs = {"outs": {"foo": File(...)}}
    #    all_output_files = [File(...), ...]
    out_attrs = ("outs", "module_outs", "implicit_outs")
    all_outs = {}
    all_output_files = []
    for attr in out_attrs:
        all_output_files += getattr(ctx.outputs, attr)
        all_outs[attr] = {}
        for out in getattr(ctx.outputs, attr):
            short_name = out.short_path[len(outdir.short_path) + 1:]
            all_outs[attr][short_name] = out
    ...
    command = ctx.attr.config[_KernelEnvInfo].setup + """
         ...
         # Archive headers in OUT_DIR
           find ${{OUT_DIR}} -name *.h -print0                          \
               | tar czf {out_dir_kernel_headers_tar}                   \
                       ...
                       --null -T -
         # Grab outputs
           {search_and_mv_output} --srcdir ${{OUT_DIR}} --dstdir {outdir} {outs} {implicit_outs} {module_outs}
        ...
        outdir = outdir.path,
        outs = " ".join(all_outs["outs"].keys()),
        implicit_outs = " ".join(all_outs["implicit_outs"].keys()),
        module_outs = " ".join(all_outs["module_outs"].keys()),
    ...
    return [
        _KernelEnvInfo(
            dependencies = ctx.attr.config[_KernelEnvInfo].dependencies +
                           all_outs["outs"].values() + all_outs["implicit_outs"].values(),
            setup = setup,
        ),
        _KernelBuildInfo(
...
def _build_modules_image_impl_common(
    ...
    for kernel_build_out in kernel_build_outs:
        if kernel_build_out.basename == "System.map":
            system_maps.append(kernel_build_out)
    if len(system_maps) != 1:
        fail("{}: dependent kernel_build {} does not have exactly 1 System.map in outs, found {}:\n  {}".format(
            ctx.label,
            kernel_build,
            len(system_maps),
            "\n  ".join([x.path for x in system_maps]),
        ))
    system_map = system_maps[0]
    ...
    command += """
             ...
             # source build_utils.sh for create_modules_staging etc.
...
def kernel_images(
    ...
    Args:
        ...
        build_boot_images: Whether to build boot images. It must be set if either `BUILD_BOOT_IMG`
          ...
          `build_initramfs` must be set to true, and `kernel_build` must be set.
    """
    all_rules = []
