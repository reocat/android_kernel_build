...
_kernel_extracted_symbols = rule(
    ...
    attrs = {
        ...
        "_extract_symbols": attr.label(default = "//build/kernel:extract_symbols", allow_single_file = True),
        "_debug_print_scripts": attr.label(default = "//build/kernel/kleaf:debug_print_scripts"),
    },
)
...
def kernel_abi(
    ...
    `bazel run <name>` updates the main `kmi_symbol_list` of `kernel_build`.
    ...
    _kernel_extracted_symbols(
        ...
        out = name + "/symbol_list",
        kmi_symbol_list_symlink = name + "/update_destination",
    )
