...
def _kernel_build_impl(ctx):
    ...
    interceptor_log = ctx.actions.declare_file("{name}/interceptor.log".format(name = ctx.label.name))
    ...
    command += """
         # Actual kernel build
           interceptor -r -l {interceptor_log} -- make -C ${{KERNEL_DIR}} ${{TOOL_ARGS}} O=${{OUT_DIR}} ${{MAKE_GOALS}}
        ...
        interceptor_log = interceptor_log.path,
    )
