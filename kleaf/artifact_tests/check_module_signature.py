...
flags.DEFINE_boolean(
    "expect_signature", None, "Whether to expect signature from the module"
)
flags.DEFINE_string("modinfo", None, "Location of modinfo")
flags.mark_flags_as_required(["dir", "module", "expect_signature", "modinfo"])
...
class CheckModuleSignatureTest(unittest.TestCase):
    ...
    def test_module_signature(self):
        ...
        for root, dirs, files in os.walk(FLAGS.dir):
            ...
            for filename in files:
                ...
                if (
                    len(file_path_parts) >= len(module_parts)
                    and file_path_parts[-len(module_parts) :] == module_parts
                ):
                    found = True
    ...
    def assert_signature(self, file_path):
        ...
        sig_id = subprocess.check_output(
            [FLAGS.modinfo, "-F", "sig_id", file_path], text=True
        ).strip()
