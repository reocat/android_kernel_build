...
class CheckModuleSignatureTest(unittest.TestCase):
    ...
    def assert_signature(self, file_path):
        # TODO(b/250667773): Use -F signature or -F signer.
        sig_hashalgo=subprocess.check_output([FLAGS.modinfo, "-F", "sig_hashalgo", file_path],
                                             text=True).strip()

        self.assertEqual(FLAGS.expect_signature,
                         not bool(sig_hashalgo),
                         "sig_hashalgo is {} but {}expecting signature".format(
                             sig_hashalgo, "" if FLAGS.expect_signature else "not "
                         ))
