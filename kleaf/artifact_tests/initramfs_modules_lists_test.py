...
class InitramfsModulesLists(unittest.TestCase):
    def _detect_decompression_cmd(self, initramfs):
        ...
        Returns:
            The command that should be used to decompress the image. The
            default compression format is assumed to be LZ4.
        """
        decompression_cmd = ["lz4", "-c", "-d", "-l"]
        gzip_magic = b'\x1f\x8b\x08'

        with open(initramfs, "rb") as initramfs_file:
            if initramfs_file.read(len(gzip_magic)) == b'\x1f\x8b\x08':
                decompression_cmd = ["gzip", "-c", "-d"]

        return decompression_cmd
    ...
    def _verify_modules_load_lists(self, modules_list_name, vendor_boot_name, image_files):
        ...
        modules_load_lists.sort()
        modules_load_list_matches.sort()

        self.assertEqual(modules_load_lists, modules_load_list_matches)
    ...
    def _verify_modules_dep_entries_exist(self, modules_lists, modules_dir):
        ...
        """
        modules_dep_path = os.path.join(modules_dir, "modules.dep")
        modules_dep_set = set()

        self.assertTrue(os.path.isfile(modules_dep_path), f"Can't find {modules_dep_path}")
        ...
        with open(modules_dep_path) as modules_dep_file:
            for line in modules_dep_file:
                ...
                modules_dep_set.add(os.path.basename(mod_name))

        for mod_list in modules_lists:
            mod_list_path = os.path.join(modules_dir, mod_list)
            self.assertTrue(os.path.isfile(mod_list_path), f"Can't find {mod_list_path}")
            mod_list_modules = set()

            with open(mod_list_path) as mod_list_file:
                for line in mod_list_file:
                    mod_list_modules.add(os.path.basename(line.strip()))
    ...
    def test_modules_dep(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for v in kernel_versions:
                modules_dir = os.path.join(lib_modules, v)
                self._verify_modules_dep_entries_exist(modules_lists, modules_dir)
