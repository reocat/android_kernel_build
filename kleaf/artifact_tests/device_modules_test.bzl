...
def _check_signature_for_modules_install(
    ...
    native.py_test(
        ...
        srcs = [script],
        python_version = "PY3",
        data = [modinfo, name + "_modules_install_staging_dir"],
        args = [
            "--dir",
            "$(location {}_modules_install_staging_dir)".format(name),
            "--module",
            base_kernel_module,
...
def _check_signature_for_initramfs(
    ...
    native.py_test(
        ...
        srcs = [script],
        python_version = "PY3",
        data = [modinfo, name + "_initramfs_staging_archive"],
        args = [
            "--archive",
            "$(location {}_initramfs_staging_archive)".format(name),
            "--module",
            base_kernel_module,
            "--expect_signature" if expect_signature else "--noexpect_signature",
            "--modinfo",
...
def _create_one_device_modules_test(
    ...
    build_config_content = """
                ...
                MAKE_GOALS="modules"
                DEFCONIFG="device_modules_test_gki_defconfig"
