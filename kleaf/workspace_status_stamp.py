...
class Stamp(object):
  ...
  def init_for_dot_source_date_epoch_dir(self) -> None:
    ...
    if self.kernel_dir:
      self.kernel_rel = self.kernel_dir.relative_to(pathlib.Path(".").resolve())
  ...
  def async_get_source_date_epoch_all(self) -> dict[str, PathCollectible]:
    ...
    return {
        proj: self.async_get_source_date_epoch(proj) for proj in all_projects
    }
  ...
  def collect_map(
    ...
    return {
        path: path_popen.collect() for path, path_popen in legacy_map.items()
    }
