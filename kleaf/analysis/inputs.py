...
class Path(object):
    """A tuple that contains a path and whether it is expected to be a directory."""
    path: pathlib.Path
...
def analyze_inputs(args):
    ...
    Args:
        args: arguments to `bazel aquery`
    """
...
def split_existing_files(files: set[pathlib.Path]):
    """Splits the given list of paths into exising and missing sets.
