...
class BuildConfigToBazelTest(unittest.TestCase):
  ...
  def _run_test(
    ...
    expected_list: list of texts expected in the final output. Each token
    may
        ...
        with self.subTest('expect output', expected=expected):
          if isinstance(expected, tuple):
            ...
            self.assertEqual(
                ...
                out.count(expected),
                (
                    f'{repr(expected)} not found {expected_count} time(s)'
                    f' in:\n{out}'
                ),
            )
          else:
            self.assertTrue(
                expected in out, f'{repr(expected)} not found in:\n{out}'
            )
