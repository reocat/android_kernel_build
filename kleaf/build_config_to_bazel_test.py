...
class BuildConfigToBazelTest(unittest.TestCase):
    ...
    def _run_test(self, name, expected_list: list[str|tuple[str, int]], expected_files, argv=()):
        """
        Args:
        ...
        with unittest.mock.patch.object(build_config_to_bazel.BuildozerCommandBuilder,
            ...
            try:
                ...
                builder._add_packge_comment_for_test = True
                builder.run()
            except Exception:
                self.stderr.seek(0)
            ...
            for expected in expected_list:
                with self.subTest('expect output', expected=expected):
                    if isinstance(expected, tuple):
                        expected, expected_count = expected
                        self.assertEqual(
