...
class BuildConfigToBazelTest(unittest.TestCase):
    ...
    def _run_test(self, name, args=()):
        os.environ["BUILD_CONFIG"] = \
            f"{TEST_DATA}/{name}"
        try:
            build_config_to_bazel.main(["--stdout"] + list(args))
        except:
            self.stderr.seek(0)
    ...
    def test_everything(self):
        ...
        self.assertIn("build.config.fragment", out)
        self.assertIn("kernel_build_config", out)
        # FAST_BUILD
        self.assertIn("--config=fast", out)
        # LTO
        self.assertIn("--lto=thin", out)
        # FILES
        ...
        self.assertIn("MODULE_OPTIONS", out)
