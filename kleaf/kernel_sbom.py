...
class KernelSbom:
  ...
  def _checksum(self, file_path: pathlib.Path) -> str:
    with file_path.open("rb") as f:
      return f'SHA1: {hashlib.file_digest(f, "sha1")
                      .hexdigest().encode(encoding="utf-8")}'

  def _generate_package_verification_code(self, files: list[File]) -> str:
    hash = hashlib.sha1()
    hash.update(''.join(sorted(f.checksum for f in files)))
  ...
  def _generate_sbom(self) -> dict[str: Any]:
    sbom = {}
    packages = []
    packages.append(self._generate_package_dict(self._android_kernel_version,
                                          _MAIN_PACKAGE_NAME,
                                          self._files))
    packages.append(self._generate_package_dict(self._android_kernel_version,
                                          _SOURCE_CODE_PACKAGE_NAME))
    packages.append(self._generate_package_dict(self._upstream_kernel_version,
  ...
  def write_sbom_file(self, output_path: pathlib.Path):
    try:
      ...
      print(f"Failed to open file {output_path} due to error {e}")
      raise
    except TypeError as e:
      print(f"Unable to serialize sbom data into json object. Error: {e}")
      raise
...
def get_file_list(dist_dir: pathlib.Path) -> Iterable[pathlib.Path]:
  if dist_dir.is_dir():
    ...
    raise FileNotFoundError(
      f"Build artifacts/Dist directory {dist_dir} doesn't exist.")
