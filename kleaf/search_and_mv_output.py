...
def search_and_mv_output_real(srcdir, dstdir, outputs):
  """
  For each output in <outputs>, searches <output> under <srcdir>, and
  ...
  missing_error = lambda out: f"No files matches {out}, expected 1"
  multiple_error = lambda out, matches: \
    f"Multiple files matches {out}, expected 1:\n  " + ("\n  ".join(matches))
  errors = []
  for out in outputs:
...
def search_and_mv_output(srcdir, dstdir, outputs):
  """
...
if __name__ == '__main__':
  parser = argparse.ArgumentParser(description=search_and_mv_output.__doc__)
  ...
  search_and_mv_output(**vars(args))
