...
def _kernel_modules_install_impl(ctx):
    ...
    if kernel_build:
        ...
        print("WARNING!: `kernel_build` is deprecated, please remove it.")
    else:
        for kernel_module in ctx.attr.kernel_modules:
            if kernel_build:
                break
            kernel_build = kernel_build or kernel_module[KernelModuleInfo].kernel_build
    if kernel_build == None:
        fail("Please provide at least one value for `kernel_modules`")
...
""",
    attrs = {
        ...
        "kernel_build": attr.label(
            ...
            doc = "[Deprecated] Label referring to the `kernel_build` module.",
        ),
