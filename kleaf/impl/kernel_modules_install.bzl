...
def _kernel_modules_install_impl(ctx):
    ...
    if ctx.attr.kernel_modules:
        if kernel_build:
            # buildifier: disable=print
            print("WARNING!: `kernel_build` IGNORED because `kernel_modules`" +
                  " was provided")
...
""",
    attrs = {
        ...
        "kernel_build": attr.label(
            ...
            doc = "Label referring to the `kernel_build` module. Only needed" +
                  "when `kernel_modules` is empty.",
        ),
