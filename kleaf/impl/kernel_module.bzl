...
def _kernel_module_impl(ctx):
    ...
    _check_kernel_build(kernel_module_deps, KernelModuleInfo, ctx.attr.kernel_build, ctx.label)

    # FIXME disallow depending on kernel_module & ddk_module that belong to the same package.
    #   Either depend on one kernel_module for a given package, or one or more ddk_modules for a given package.
    ...
    # FIXME custom Module.symvers name does not work with kernel_mdoules_install
    setup = """
