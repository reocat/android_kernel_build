...
def _dependency_graph_impl(ctx):
    out = ctx.actions.declare_file("{}/dependency_graph".format(ctx.attr.name))
    ...
    tools = [ctx.executable._dependency_graph]
    ...
    command += """
        ...
        {dependency_graph} {flags} {intermediates_dir}
        rm -rf {intermediates_dir}
    """.format(
        srcs = " ".join([file.path for file in srcs]),
        intermediates_dir = intermediates_dir,
        dependency_graph = ctx.executable._dependency_graph.path,
        flags = " ".join(flags),
    ...
    ctx.actions.run_shell(
        ...
        mnemonic = "KernelDependencyGraph",
    )

    return DefaultInfo(files = depset([out]))

dependency_graph = rule(
    ...
    attrs = {
        ...
        "_dependency_graph": attr.label(
            default = "//build/kernel:dependency_graph",
            cfg = "exec",
            executable = True,
        ),
