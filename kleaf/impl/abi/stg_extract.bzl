...
def _stgextract_impl(ctx):
    ...
    abi_out_file = ctx.actions.declare_file("{}/{}.stg".format(ctx.attr.name, ctx.attr.name))
    args.append("--output {abi_out_file}".format(abi_out_file = abi_out_file.file.path))
    ...
    # Separate files based on their suffix.
