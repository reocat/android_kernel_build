...
def _protected_exports_impl(ctx):
    ...
    if not ctx.attr.protected_modules_list:
        fail("{}: requires protected_modules_list.".format(ctx.label))

    out = ctx.actions.declare_file("{}/protected_exports".format(ctx.attr.name))
    intermediates_dir = utils.intermediates_dir(ctx)

    inputs = [
        ctx.file.protected_modules_list,
    ]
    ...
    flags += ["--gki-protected-modules-list", ctx.file.protected_modules_list.path]
...
protected_exports = rule(
    ...
    attrs = {
        ...
        "protected_modules_list": attr.label(doc = "List of protected modules whose exports needs to be extracted.", allow_single_file = True),
