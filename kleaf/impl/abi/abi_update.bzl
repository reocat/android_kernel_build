...
abi_update = rule(
    ...
    attrs = {
        ...
        "nodiff_update": attr.label(
            ...
            executable = True,
            # HACK: Use target platform for the executable to avoid applying the
            # exec platform transition on kernel_build, causing it to build
            # again. The executable itself is executed on the exec platform
            # in this rule.
            # TODO(b/287162898): clean this up when nodiff_update is its own rule, with
            # target platform transition.
            cfg = "target",
        ),
        "diff": attr.label(
            ...
            # again. The executable itself is executed on the exec platform
            # in this rule.
            # TODO(b/287162898): clean this up when diff is its own rule, with
            # target platform transition.
            cfg = "target",
        ),
    },
