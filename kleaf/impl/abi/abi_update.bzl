...
def _abi_update_impl(ctx):
    ...
    script = """
        ...
        if [[ $1 == "--commit" ]] || [[ $1 == "--print_git_commands" ]]; then
            echo "  git -C {abi_definition_dir} add $(basename {abi_definition_name})"
            echo "  git -C {abi_definition_dir} commit -F {git_message}"
            echo "  git -C {abi_definition_dir} commit --amend"
        fi
    ...
    """.format(
        ...
        abi_definition_dir = paths.dirname(ctx.file.abi_definition_stg.short_path),
        abi_definition_name = paths.basename(ctx.file.abi_definition_stg.short_path),
