...
def _stgdiff_impl(ctx):
    ...
    compare_options = " ".join(
        ["--compare-option {option}".format(
            option = option,
        ) for option in STGDIFF_COMPARE_OPTIONS],
    )

    # TODO(b/261722616): Check if  missing support for symbol list filter is needed.
    command = ctx.attr._hermetic_tools[HermeticToolsInfo].setup + """
        set +e
        {stgdiff}  --stg {new} {baseline}       \\
                   {compare_options}            \\
                   {outputs} > {error_msg_file} 2>&1
        rc=$?
        set -e
