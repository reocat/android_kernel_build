...
def _get_grab_kernel_output_step(ctx):
    ...
    outputs = []
    if ctx.attr._get_kernel_output[BuildSettingInfo].value:
        kernel_output_target = ctx.actions.declare_directory("{name}/get_kernel_output".format(name = ctx.label.name))
        outputs.append(kernel_output_target)
        grab_kernel_output_cmd = """
            rsync -a --prune-empty-dirs --include '*/' ${{OUT_DIR}}/ {kernel_output_target}/
        """.format(
