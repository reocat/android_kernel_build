...
def _repack_modules_staging_archive(
    ...
    cmd = ctx.attr._hermetic_tools[HermeticToolsInfo].setup + """
        ...
        base_module_paths=$(tar tf {base_archive} | grep '[.]ko$' || true)
        base_module_names=$(mktemp)
        echo "${{base_module_paths}}" | xargs -r -n 1 basename | sort > ${{base_module_names}}
        ...
        tar xf {base_archive} -X {all_module_basenames_file} -C {modules_staging_dir} ${{base_module_paths}}

        # Move base_kernel (GKI) modules to the directory of device modules
        base_kernel_release=$(basename {modules_staging_dir}/lib/modules/*)
        device_kernel_release=$(cat {kernel_release_file})
        if [[ ${{base_kernel_release}} != ${{device_kernel_release}} ]]; then
            mv {modules_staging_dir}/lib/modules/"${{base_kernel_release}}" {modules_staging_dir}/lib/modules/"${{device_kernel_release}}"
        fi

        # Extract device modules, excluding all modules matching the name of a GKI module,
        # but including unprotected modules overriden by the device
