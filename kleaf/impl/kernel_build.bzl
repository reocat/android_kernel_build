...
def _get_grab_cmd_step(ctx):
    """Returns a step for grabbing the `*.cmd` from `OUT_DIR`.
...
def _build_main_action(
    ...
    grab_cmd_step = _get_grab_cmd_step(ctx)
    steps = (
        interceptor_step,
        cache_dir_step,
...
def _create_infos(
    ...
    cmds_info = KernelCmdsInfo(directory = main_action_ret.cmd_dir)
