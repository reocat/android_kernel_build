...
def kernel_build(
    ...
    Args:
        ...
        sanitizer: Default is `None`. Sanitizer configuration used during kernel build.
          Value may be one of `"kasan"`, `"kasan_sw_tags"`, or `"kcsan"`.
          By default, no sanitizer will be used.
    ...
    defconfig_fragments = _get_defconfig_fragments(
        ...
        kernel_build_sanitizer = sanitizer,
        **internal_kwargs
    )
...
def _get_defconfig_fragments(
    ...
    file_selector(
        ...
        first_selector = select({
            Label("//build/kernel/kleaf:kasan_any_mode_is_true"): "//build/kernel/kleaf/impl/defconfig:kasan_any_mode",
            Label("//build/kernel/kleaf:kasan_sw_tags_is_true"): "//build/kernel/kleaf/impl/defconfig:{}_kasan_sw_tags".format(kernel_build_arch),
            Label("//build/kernel/kleaf:kcsan_is_true"): "//build/kernel/kleaf/impl/defconfig:kcsan",
            "//conditions:default": None,
        }),
        second_selector = kernel_build_sanitizer,
        third_selector = "default",
        files = {
            Label("//build/kernel/kleaf/impl/defconfig:kasan_any_mode_defconfig"): "kasan",
            Label("//build/kernel/kleaf/impl/defconfig:{}_kasan_sw_tags_defconfig".format(kernel_build_arch)): "kasan_sw_tags",
            Label("//build/kernel/kleaf/impl/defconfig:kcsan_defconfig"): "kcsan",
            Label("//build/kernel/kleaf/impl:empty_filegroup"): "default",
        },
        **internal_kwargs
    )
...
def _kmi_symbol_list_strict_mode(ctx, all_output_files, all_module_names_file):
    ...
    if ctx.attr.sanitizer and ctx.attr.sanitizer != "default":
        # buildifier: disable=print
        print("\nWARNING: {this_label}: Attribute kmi_symbol_list_strict_mode\
              IGNORED because kernel_build.sanitizer is set!".format(this_label = ctx.label))
        return None
