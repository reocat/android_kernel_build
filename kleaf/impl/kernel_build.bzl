...
def kernel_build(
    ...
    defconfig_fragments = _get_defconfig_fragments(
        ...
        kernel_build_sanitizer = sanitizer if sanitizer else "default",
        **internal_kwargs
    )
...
def _skip_build_checks(ctx, what):
    ...
    if ctx.attr._sanitizer[BuildSettingInfo].value != "default":
        print("\nWARNING: {this_label}: {what} was\
              IGNORED because sanitizer is set!".format(this_label = ctx.label, what = what))
...
def _get_defconfig_fragments(
    ...
    file_selector(
        ...
        third_selector = "default",
        files = {
            Label("//build/kernel/kleaf/impl/defconfig:kasan_enabled_defconfig"): "kasan_generic",
            Label("//build/kernel/kleaf/impl/defconfig:kasan_sw_tags_enabled_defconfig"): "kasan_sw_tags",
            Label("//build/kernel/kleaf/impl/defconfig:kcsan_enabled_defconfig"): "kcsan",
            Label("//build/kernel/kleaf/impl:empty_filegroup"): "default",
        },
        **internal_kwargs
    )
