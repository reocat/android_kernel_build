    ...
    for attr, val in attr_vals.items():
        if _is_auto_attr_list(ctx, attr, val):
            all_output_files[attr] = {
                "_auto_key": ctx.actions.declare_directory("{}/{}".format(ctx.label.name, attr)),
            }
        else:
            all_output_files[attr] = {
...
def _is_auto_output_modules(attr, name_to_file_mapping):
    """Given a key value pair in `all_output_files`, returns true iff it refers to an auto modules list.

    This is similar to `_is_auto_attr_list` but it operates on the constructed dictionary of
    outputs, `all_output_files`.
    """
    return bool(_get_auto_module_output_dir(attr, name_to_file_mapping))
...
def _get_check_remaining_modules_step(
    ...
    skip_check = any([_is_auto_output_modules(attr, d) for attr, d in all_output_files.items()])
...
def _create_infos(
    ...
    output_group_kwargs = {}
    for attr, d in all_output_files.items():
        if not _is_auto_output_modules(attr, d):
