...
_kernel_build = rule(
    ...
    attrs = {
        "config": attr.label(
            ...
            # All deps of kernel_build unsets trim configs.
            # kernel_config calculates trim configs on its own.
        ...
        "modules_prepare": attr.label(
            ...
            # All deps of kernel_build unsets trim configs.
            # modules_prepare calculates trim configs on its own.
            cfg = trim_nonlisted_kmi_utils.unset_transition,
        ),
...
def _kmi_symbol_list_violations_check(ctx, modules_staging_archive):
    ...
    inputs += ctx.attr.config[0][KernelBuildOriginalEnvInfo].env_info.dependencies
    ...
    command = ctx.attr.config[0][KernelBuildOriginalEnvInfo].env_info.setup
    command += """
