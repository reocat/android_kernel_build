...
def _create_filegroup_def_archive(
        ctx,
        main_action_ret,
        all_module_names_file,
        modules_staging_archive,
    ...
    deps_repr = repr([":{}".format(file.basename) for file in deps_files] + [
        # unstripped modules is generated by another rule, so it is downloaded separately.
        "//{}".format(UNSTRIPPED_MODULES_ARCHIVE),
        "//{}".format(ctx.attr.name + "_ddk_headers_archive.tar.gz"),
    ])
...
""".format(
    ...
    transitive_inputs = [target.files for target in ctx.attr.ddk_module_defconfig_fragments]
    transitive_inputs.append(ctx.attr.kernel_uapi_headers.files)
    inputs = depset(
        direct_inputs,
    ...
    # TODO(b/291918087): Move the list of files needed by the kernel_filegroup
    # from DefaultInfo to here.
...
def _kernel_build_impl(ctx):
    ...
    filegroup_def_archive = _create_filegroup_def_archive(
        ctx = ctx,
        main_action_ret = main_action_ret,
        all_module_names_file = all_module_names_file,
        modules_staging_archive = modules_staging_archive,
    ...
    infos = _create_infos(
        ...
        module_srcs = module_srcs,
        internal_outs_archive = internal_outs_archive,
        filegroup_def_archive = filegroup_def_archive,
    )
