...
def _get_cache_dir_step(ctx):
    ...
    return struct(inputs = [], cmd = cache_dir_cmd, outputs = [])
...
def _get_grab_unstripped_modules_step(ctx, all_module_names):
    ...
    outputs = []
    unstripped_dir = None
    if all_module_names and ctx.attr.collect_unstripped_modules:
        all_module_basenames = [paths.basename(filename) for filename in all_module_names]
        all_module_basenames_file = _write_module_names_to_file(ctx, "all_module_basenames.txt", all_module_basenames)
...
def _build_main_action(
    ...
    inputs += kbuild_mixed_tree_ret.files
    for step in steps:
        inputs += step.inputs
...
def _create_infos(
    ...
    env_info_dependencies += kbuild_mixed_tree_ret.files
...
def _kernel_build_impl(ctx):
    ...
    all_module_names_file = _write_module_names_to_file(ctx, ctx.label.name + MODULE_OUTS_FILE_SUFFIX, all_output_names.modules)
    ...
    kmi_strict_mode_out = _kmi_symbol_list_strict_mode(ctx, main_action_ret.all_output_files, all_module_names_file)
    ...
    infos = _create_infos(
        ...
        all_module_names_file = all_module_names_file,
        unstripped_dir = main_action_ret.unstripped_dir,
