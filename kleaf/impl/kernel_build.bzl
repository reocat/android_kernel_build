...
def _get_grab_gcno_step(ctx):
    ...
    if ctx.attr._gcov[BuildSettingInfo].value:
        ...
        gcno_zip = ctx.actions.declare_file(
            "{name}/gcno.tar.gz".format(name = ctx.label.name),
        )
        outputs += [gcno_dir, gcno_mapping, gcno_zip]
