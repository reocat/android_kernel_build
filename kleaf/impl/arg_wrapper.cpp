...
DieErrorHelper die() { return DieErrorHelper(false); }

DieErrorHelper die_error() { return DieErrorHelper(true); }
...
int execv(const std::filesystem::path& executable,
  ...
  return ::execv(executable.c_str(), cargv.data());
}
}  // namespace
...
int main(int argc, char* argv[]) {
  ...
  if (-1 != execv(real_executable, &new_argv)) {
