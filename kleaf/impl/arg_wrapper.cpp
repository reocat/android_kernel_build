...
[[noreturn]]
__attribute__((__format__(__printf__, 1, 2))) void die(const char* fmt, ...) {
  va_list args;
...
std::filesystem::path get_kleaf_internal_dir() {
  ...
  if (ec.value() != 0) {
    die("ERROR: read_symlink /proc/self/exe: %s", ec.message().c_str());
  }
...
std::vector<std::string> load_arg_file(const std::filesystem::path& path) {
  ...
  if (ifs.fail()) {
    die("Unable to open %s: %s", path.c_str(), strerror(errno));
  }
 ...
 public:
  [[noreturn]] void Exec(const std::filesystem::path& executable) {
    ...
    if (-1 != execv(executable.c_str(), cargv.data())) {
      die("ERROR: execv %s: %s", executable.c_str(), strerror(errno));
    }
