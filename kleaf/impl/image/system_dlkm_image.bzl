...
def _system_dlkm_image_impl(ctx):
    ...
    outputs_to_compare = []
    if system_dlkm_fs_type:
        outputs_to_compare = ["system_dlkm.img"]
    else:
        outputs_to_compare = ["system_dlkm.{}.img".format(fs_type) for fs_type in system_dlkm_fs_types]
    utils.compare_file_names(
        default_info.files.to_list(),
        SYSTEM_DLKM_OUTS + outputs_to_compare,
