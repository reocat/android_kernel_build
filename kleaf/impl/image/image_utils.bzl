...
def _ramdisk_options(lz4_ramdisk, lz4_ramdisk_compress_args):
    """Options for how to treat ramdisk images.

    Args:
        lz4_ramdisk: if set to `True`, any ramdisks generated will be lz4
         compressed instead of gzip compressed. A `None` value here means
         a fallback value from a build.config file will be used.
    ...
    if lz4_ramdisk:
        ...
        if lz4_ramdisk_compress_args:
            _ramdisk_compress += lz4_ramdisk_compress_args
        else:
            _ramdisk_compress += "-12 --favor-decSpeed"
        _ramdisk_decompress = "lz4 -c -d -l"
    elif lz4_ramdisk != None:
