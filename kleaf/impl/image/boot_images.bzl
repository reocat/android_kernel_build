...
def _boot_images_impl(ctx):
    ...
    if ctx.attr.avb_sign_boot_img:
        if ctx.attr.avb_boot_partition_size == 0 or \
           ctx.attr.avb_boot_key == None or ctx.attr.avb_boot_algorithm == "" or \
           ctx.attr.avb_boot_partition_name == "":
...
""", values = ["vendor_boot", "vendor_kernel_boot"]),
        ...
        "avb_boot_key": attr.label(
            doc = """ Path to the key used for signing.
