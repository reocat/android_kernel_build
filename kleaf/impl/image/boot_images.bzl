...
def _boot_images_impl(ctx):
    ...
    if ctx.attr.initramfs:
        ...
        """
    if ctx.attr.skip_unpacking_ramdisk:
        boot_flag_cmd += """
...
""", values = ["vendor_boot", "vendor_kernel_boot"]),
        ...
        "skip_unpacking_ramdisk": attr.bool(
            doc = """ If set, skip unpacking the vendor ramdisk and copy it as
            is, without modifications, into the boot image.
            Also skip the mkbootfs step.""",
        ),
