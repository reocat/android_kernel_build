...
def kernel_images(
    ...
    Args:
        ...
        boot_image_outs: A list of output files that will be installed to `DIST_DIR` when
          ...
          - `RAMDISK_EXT=lz4`. Is used when `lz4_ramdisk`(see below) is not specified.
    ...
    if build_initramfs:
        ...
        if lz4_ramdisk_compress_args and not lz4_ramdisk:
            fail('"lz4_ramdisk_compress_args" was provided but "lz4_ramdisk" was not set.')
