...
def _get_kernel_release(ctx):
    ...
    command = ctx.attr._hermetic_tools[HermeticToolsInfo].setup + """
        ...
        if [[ -z "${{kernel_release}}" ]]; then
            # TODO(b/284198523): Remove legacy method
            kernel_release=$(cat {gki_info} | sed -nE 's/^certify_bootimg_extra_args=.*KERNEL_RELEASE:(\\S+)/\\1/p')
            echo "WARNING: Inferring kernel_release from certify_bootimg_extra_args: ${{kernel_release}}" >&2
            echo "  Please use a newer build as this method will not be supported in the future." >&2
        fi
        if [[ -z "${{kernel_release}}" ]]; then
