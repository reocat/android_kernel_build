...
def _kernel_filegroup_impl(ctx):
    ...
    ddk_config_env_setup_script = ctx.actions.declare_file("{name}/{name}_ddk_config_setup.sh")
    ctx.actions.write(
    ...
    ddk_config_env = KernelSerializedEnvInfo(
        ...
        # FIXME also module_scripts and module_kconfig
        tools = depset([
            # FIXME SOURCE DEP!
            ctx.file._build_utils_sh,
        ], transitive = [
            hermetic_tools.deps,
        ]),  # FIXME also clang toolchain
    )
