...
kernel_filegroup(
    ...
    config_out_dir_files = glob([{config_out_dir_repr} + "/**"], allow_empty = True),
...
def _write_filegroup_decl_file(ctx, info, deps_files, kernel_uapi_headers, template_file):
    ...
    # For local files in this package, prefixed with ":"
    pkg = dict(
        allow_closure = True,
    ...
    sub.add_joined("{modules_prepare_archive_repr}", depset([info.modules_prepare_archive]), **(one | pkg))
