...
def _kernel_filegroup_declaration_impl(ctx):
    ...
    deps_repr = repr([":{}".format(file.basename) for file in deps_files] + [
        ...
        "//{}".format(ctx.attr.kernel_build.label.name + "_ddk_headers_archive.tar.gz"),
    ])
...
""".format(
        ...
        ddk_module_defconfig_fragments_repr = files_to_pkg_label(info.ddk_module_defconfig_fragments.to_list()),
    ...
    filegroup_decl_file = ctx.actions.declare_file("{}/{}".format(ctx.attr.kernel_build.label.name, FILEGROUP_DEF_TEMPLATE_NAME))
