...
while [[ $# -gt 0 ]]; do
  ...
  --help)
    ...
    echo "      Use absolute path if overriding in the command line."
    echo "      If relative, it is interpreted against execroot."
    echo "      Relative paths should only be set by checkpatch() Bazel rule."
    echo
...
if [[ -n "${IGNORELIST_FILE}" ]]; then
  if [[ -f "${IGNORELIST_FILE}" ]]; then
    ...
    echo "ERROR: --ignored_checks is not a file: $(realpath -m ${IGNORELIST_FILE})" >&2
    exit 1
  fi
fi
