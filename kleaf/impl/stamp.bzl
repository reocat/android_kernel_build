...
def _get_scmversion_cmd(ctx, src_path):
    # {path}:{scmversion} {path}:{scmversion} ...
    cmd = status.get_stable_status_cmd(ctx, "STABLE_SCMVERSIONS")
    cmd += """ | sed -n 's|.*\\<'{quoted_src_path}':\\(\\S\\+\\).*|\\1|p'""".format(
        quoted_src_path = shell.quote(src_path),
    )
...
def _write_localversion_step(ctx, out_path):
    ...
    if ctx.attr._config_is_stamp[BuildSettingInfo].value:
        deps = [ctx.info_file]
        stable_scmversion_cmd = _get_scmversion_cmd(ctx, "${KERNEL_DIR}")
    else:
        deps = []
...
def _get_ext_mod_scmversion(ctx, ext_mod):
    ...
    scmversion_cmd = _get_scmversion_cmd(ctx, ext_mod)
