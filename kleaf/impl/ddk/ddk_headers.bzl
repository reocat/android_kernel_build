...
def ddk_headers_common_impl(label, hdrs, includes):
    ...
    for include_dir in includes:
        ...
        if paths.normalize(include_dir) != include_dir:
            fail("{}: include directory {} is not normalized to {}".format(label, include_dir, paths.normalize(include_dir)))
        if paths.is_absolute(include_dir):
            fail("{}: Absolute dirs not allowed in export_include_dirs: {}".format(label, include_dir))
    ...
    ret_includes = depset([paths.join(label.package, d) for d in includes], transitive = transitive_includes)
