...
class GenDdkHeaders(buildozer_command_builder.BuildozerCommandBuilder):
  ...
  def _handle_unresolved(self):
    ...
    if self._include_data.unresolved:
      logging.error(
          "Found unresolved includes. Run with `--dump` to trace the sources."
      )
  ...
  def _generate_target(
    ...
    Args:
        ...
        files: List of files to be included in the given ddk_headers target as
          `hdrs`
        includes_attr: Name of the attribute for includes directories. Can be
          `includes` or `linux_includes`. See doc for ddk_headers for details.
        ...
        should_include: A callable that, given a path to a file or directory
          relative to the package, returns True if the file or directory should
          be placed in this ddk_headers target, or False otherwise.
    """
