...
def jsonify(object):
    ...
    if isinstance(object, dict):
        return collections.OrderedDict(sorted((str(key), jsonify(value)) for key, value in object.items()))
    return str(object)
...
class FuzzySearchResult(object):
    ...
    known_files_reversed: dict[pathlib.Path, list[pathlib.Path]] = dataclasses.field(default_factory=dict)
    all_headers_reversed: dict[pathlib.Path, list[pathlib.Path]] = dataclasses.field(default_factory=dict)
...
class GenDdkHeaders(buildozer_command_builder.BuildozerCommandBuilder):
    ...
    async def _fuzzy_resolve(self, all_headers: list[pathlib.Path],
        ...
        for file, raw_includes in file_raw_includes.items():
            files.append(file)
            awaitables.append(self._fuzzy_resolve_multi(raw_includes, known_files_reversed, all_headers_reversed))
            ...
            for result in results:
                if result:
                    ...
                    for possible_child in result.files:
                        possible_include_dir = pathlib.Path(*possible_child.parts[:-len(result.raw_include.parts)])
                        if possible_include_dir in known_include_dirs:
                            ret_files.add(possible_child)
                            ret_includes.add(possible_include_dir)
                            found_matching = True
                            break
                    if not found_matching:
                        # Can't find the file in existing include directories, so this is a new
                        # finding. Add all matched files.
                        ret_files |= set(result.files)
                        ret_includes |= set(pathlib.Path(*child.parts[:-len(result.raw_include.parts)]) for child in result.files)
                else:
    ...
    async def _fuzzy_resolve_multi(self, raw_includes: Iterable[pathlib.Path],
        ...
        for raw_include in raw_includes:
            res.append(self._fuzzy_resolve_one(raw_include, known_headers_reversed, all_headers_reversed))
        return res

    def _fuzzy_resolve_one(self, want: pathlib.Path, known_reversed: dict[pathlib.Path, list[pathlib.Path]],
                           fallback_reversed: dict[pathlib.Path, list[pathlib.Path]]) -> FuzzySearchOneResult:
