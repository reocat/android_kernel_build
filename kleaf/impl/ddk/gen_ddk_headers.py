...
def merge_paths_with_sources(
    self: PathsWithCompUnitType, other: PathsWithCompUnitType
...
class IncludeDataWithSource(object):
    ...
    include_dirs: PathsWithCompUnitType = dataclasses.field(
        default_factory=PathsWithCompUnit
    )
    include_files: PathsWithCompUnitType = dataclasses.field(
        default_factory=PathsWithCompUnit
    )
    unresolved: PathsWithCompUnitType = dataclasses.field(
        default_factory=PathsWithCompUnit
    )
    ...
    def from_dict(d, source):
        ...
        ret.include_dirs = {
            pathlib.Path(item): {source} for item in d["include_dirs"]
        }
        ret.include_files = {
            pathlib.Path(item): {source} for item in d["include_files"]
        }
        ret.unresolved = {
            pathlib.Path(item): {source} for item in d["unresolved"]
        }
        return ret
...
class GenDdkHeaders(buildozer_command_builder.BuildozerCommandBuilder):
    ...
    def __init__(
        self, include_data: IncludeDataWithSource, *init_args, **init_kwargs
    ):
    ...
    def _sanitize_includes(
        self, d: dict[pathlib.Path, Any]
    ) -> dict[pathlib.Path, Any]:
    ...
    def _calc(self):
        ...
        * Step 1: Sanitize output from step 0, filtering out source files that
        does
          not exist in the workspace
        * Step 2: Categorize the outputs from step 0 by their package. The
        package
          is guessed heuristically.
        """
        for k, v in vars(self._include_data).items():
            self._dump_debug(
                pathlib.Path("0_input", k).with_suffix(".json"), jsonify(v)
            )
        ...
        self._sanitized_include_data = IncludeDataWithSource(
            include_files=self._sanitize_includes(
                self._include_data.include_files
            ),
            include_dirs=self._sanitize_includes(
                self._include_data.include_dirs
            ),
        )
        self._dump_debug(
            "1_sanitized/input_sanitized.json",
        ...
        if self._outside:
            ...
            logging.error(
                "The following are outside of repo: \n%s", "\n".join(strs)
            )
        ...
        ) in self._sanitized_include_data.include_files.items():
        ...
        self._dump_debug(
            "2_calc/package_files.json", jsonify(self._package_files)
        )
        ...
        for (
            include,
            source_compile_units,
        ) in self._sanitized_include_data.include_dirs.items():
        ...
        self._dump_debug(
            "2_calc/package_includes.json", jsonify(self._package_includes)
        )

        self._dump_debug(
            "2_calc/unknown_package.json", jsonify(self._unknown_package)
        )
        if self._unknown_package:
            strs = sorted(str(path) for path in self._unknown_package)
            logging.info(
                "The following paths have unknown packages: \n%s",
                "\n".join(strs),
            )
    ...
    def _handle_unresolved(self):
        ...
        There should not be any. If there are, We don't find a good way to
        handle
        these yet, so let's output an error message and asks for manual
        intervention.
        """
        if self._include_data.unresolved:
            logging.error(
                "Found unresolved includes. Run with `--dump` to trace the"
                " sources."
            )
    ...
    def _create_buildozer_commands(self):
        ...
        for package, files in sorted_package_files:
            self._generate_target(
                package,
                f"all_headers_allowlist_{self.args.arch}",
                files,
                "linux_includes",
    ...
    def _generate_target(
        ...
        Args:
            ...
            files: List of files to be included in the given ddk_headers target
              as `hdrs`
            includes_attr: Name of the attribute for includes directories. Can
              be `includes` or `linux_includes`. See doc for ddk_headers for
              details.
            include_dirs: List of include directories to put in `includes_attr`
            should_include: A callable that, given a path to a file or directory
              relative to the package, returns True if the file or directory
              should be placed in this ddk_headers target, or False otherwise.
        """
        ...
        if glob_dirs:
            glob_target = self._new(
                "filegroup", name + "_globs", str(package), load_from=None
            )
            ...
            for glob_dir, files in glob_dirs.items():
                logging.info(
                    "%s has %d files, globbing %d files",
                    glob_dir,
                    len(files),
    ...
    def _get_glob_dir_or_none(self, rel_file) -> Optional[pathlib.Path]:
        """If --glob already contains the given rel_file, return the glob directory.
    ...
    def _is_excluded(self, item: pathlib.Path):
        return any(
            re.search(pattern, str(item)) for pattern in self.args.exclude_regex
        )

    def _is_allowed(self, item: pathlib.Path):
        return any(
            item.is_relative_to(allowed) for allowed in self.args.allowed
        )
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "-i",
        "--input",
        help="Input directory or file from analyze_inputs",
        type=pathlib.Path,
    )
    parser.add_argument(
...
def get_all_files_and_includes(path: pathlib.Path) -> IncludeDataWithSource:
    """Merge all from args.input, tracking the source too.

    Return values are un-sanitized.
    """
    if path.is_file():
        with open(path) as f:
        ...
        for root, _, files in os.walk(path):
            for file in files:
                with open(pathlib.Path(root, file)) as f:
                    ret |= IncludeDataWithSource.from_dict(
                        json.load(f), pathlib.Path(root, file)
                    )
        return ret
