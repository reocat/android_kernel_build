...
import re
import sys
from typing import Sequence, Optional, Iterable, Any, NoReturn
...
class GenDdkHeaders(buildozer_command_builder.BuildozerCommandBuilder):
    ...
    def _calc(self):
        ...
        for file in self._sanitized_include_data.include_files:
            ...
            if not package:
                continue
            self._add_except_excluded(self._package_files, package, file)
        ...
        for include in self._sanitized_include_data.include_dirs:
            ...
            if not package:
                continue
            self._add_except_excluded(self._package_includes, package, include)
    ...
    def _generate_target(self, package: pathlib.Path, name: str,
        ...
        for directory in include_dirs:
            self._add_attr(target, "includes", directory.relative_to(package), quote=True)
                ...
                if result:
                    ...
                    for possible_child in result.files:
                        ...
                        if possible_include_dir in known_include_dirs:
                            self._add_except_excluded(ret_files, possible_child, file)
                            self._add_except_excluded(ret_includes, possible_include_dir, file)
                            found_matching = True
                            break
                    if not found_matching:
                        ...
                        for child in result.files:
                            self._add_except_excluded(ret_files, child, file)
                            include_dir = prefix_of(child, result.raw_include)
                            self._add_except_excluded(ret_includes, include_dir, file)
                else:
    ...
    def _add_except_excluded(self, paths_with_sources: PathsWithSourcesType,
                             item: pathlib.Path, source: pathlib.Path):
        """Adds item to paths_with_sources if it is not excluded.

        paths_with_sources[item] is only evaluated if not excluding it.
        """
        excluded = any(re.search(pattern, str(item)) for pattern in self.args.exclude_regex)
        if not excluded:
            paths_with_sources[item].add(source)
