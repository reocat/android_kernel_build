...
def _handle_copt(ctx, expand_targets):
        ...
        if copt != expanded:
            if not copt.startswith("$(") or not copt.endswith(")") or \
               copt.count("$(") > 1:
                ...
                fail(
                    """{}: {} is not allowed. An $(location) expression must be its own
                       item. For example, Instead of specifying "-include=$(location X)",
...
def _makefiles_impl(ctx):
    ...
    expand_targets = \
        ctx.attr.module_srcs + \
        ctx.attr.module_hdrs + \
        ctx.attr.module_deps

    local_defines = [
        ctx.expand_location(define, targets = expand_targets)
        for define in ctx.attr.module_local_defines
    ]
    args.add_all("--local-defines", local_defines)

    copt_file = _handle_copt(ctx, expand_targets)
