...
def _gen_kbuild_dir(ctx, makefile):
    ...
    for hdr in ctx.attr.module_hdrs:
        include_dirs += hdr[DdkHeadersInfo].exported_include_dirs
    include_dirs_txt = ctx.actions.declare_file("{}/include_dirs.txt".format(ctx.attr.name))
    ctx.actions.write(include_dirs_txt, content = "\n".join(include_dirs))
    ...
    command += """
             # Generate Makefile for DDK module
               {gen_makefile} \\
                 ...
                 --package {package} \\
                 --ccflags_txt {ccflags_txt} \\
                 --include_dirs_txt {include_dirs_txt} \\
    """.format(
        ...
        include_dirs_txt = include_dirs_txt.path,
    )
