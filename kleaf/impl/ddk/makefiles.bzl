...
def _makefiles_impl(ctx):
    ...
    for dep in ctx.attr.module_deps:
        ...
        if DdkHeadersInfo not in dep:
            fail("{}: {} is not a valid item in deps. Only kernel_module, ddk_module, ddk_headers are allowed.".format(
                module_label,
                dep.label,
            ))
    ...
    # and my includes.
    # We could get the same DdkHeadersInfo from the target of module_label,
    ...
    args.add_all("--include-dirs", this_includes, uniquify = True)
