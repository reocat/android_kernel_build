...
def _gen_makefile(ctx):
    ...
    deps = sets.make([dep for module in ctx.attr.deps for dep in module[DdkModuleInfo].deps])
...
def _gen_kbuild_dir(ctx, makefile):
    ...
    for module in ctx.attr.deps:
        sanitized_module = utils.sanitize_label_as_filename(module.label)
        module_srcs_txt = ctx.actions.declare_file("{}/{}/module_srcs.txt".format(ctx.attr.name, sanitized_module))
        ctx.actions.write(
            module_srcs_txt,
            content = "\n".join([file.short_path for src in module[DdkModuleInfo].srcs for file in src.files.to_list()]),
        )

        ccflags = []

        for hdr in module[DdkModuleInfo].hdrs:
            for d in hdr[DdkHeadersInfo].exported_include_dirs:
                ccflags.append("-I$(srctree)/$(src)/{}/{}".format(rel_root, d))

        ccflags_txt = ctx.actions.declare_file("{}/{}/ccflags.txt".format(ctx.attr.name, sanitized_module))
        ...
        """.format(
            ...
            module_out = module[DdkModuleInfo].out,
