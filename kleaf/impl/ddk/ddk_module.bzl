...
def ddk_module(
    ...
    **The best practice is to not have conflicting header names and search paths.**
    ...
    1. Traverse depedencies for `linux_includes`:
      1. All `linux_includes` of `deps`, in the specified order (recursively apply #1.2 on each target)
      2. All `linux_includes` of `hdrs`, in the specified order (recursively apply #1.2 on each target)
    ...
    3. Traverse depedencies for `includes`:
      ...
      2. All `deps`, in the specified order (recursively apply #3.1 and #3.3 on each target)
      3. All `hdrs`, in the specified order (recursively apply #3.1 and #3.3 on each target)
    ...
    ```
    # 1.1 deps, linux_includes, recursively
    -Iuapi/dep_a

    # 1.2 hdrs, linux_includes, recursively
    -Iuapi/hdrs_a
    ...
    Args:
        ...
        linuxinclude: See [`ddk_headers.linuxinclude`](#ddk_headers-linuxinclude)

          Because `includes` are re-exported to dependent modules, setting this attribute
          to `True` causes `includes` to be added to `LINUXINCLUDE` for this module and
          dependent modules.
