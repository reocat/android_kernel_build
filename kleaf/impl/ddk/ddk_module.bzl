...
def ddk_module(
    ...
    if srcs == None:
        # FIXME better headers mechanism for local includes
        ddk_headers(
            name = name + "_internal_headers",
            srcs = native.glob(
                ["**/*.h"],
            ),
            # We don't need to add `-I` for local includes! These are, really, just plain sources
            # that does not need makefile generation.
            export_include_dirs = [],
        )
