    ...
    Args:
        ...
        local_defines: List of defines to add to the compile line.
          ...
          **Order matters**. To prevent buildifier from sorting the list, use the
          ...
          `$(location)` expressions are expanded to paths relative to the Bazel
          [workspace](https://bazel.build/concepts/build-ref#workspace).

          Each string is treated as a single Bourne shell token. Each string is
