...
def list_to_repr(lst: list[pathlib.Path], indent=None, each_indent=None):
    """Similar to repr(lst), but prettified & use double quotes."""
    result = f"["
    if lst:
        result += "\n"
    result += f"".join(f'{each_indent}{indent}"{elem}",\n' for elem in sorted(lst))
    if lst:
        result += f"{each_indent}"
    result += "]"
    return result
...
def gen_ddk_headers_archive_build_file(
    ...
    """).format(
        name=name,
        hdrs_repr=list_to_repr(hdrs, indent=indent, each_indent=indent),
        linux_includes_repr=list_to_repr(
            linux_includes, indent=indent, each_indent=indent),
        includes_repr=list_to_repr(
            includes, indent=indent, each_indent=indent),
    ))
