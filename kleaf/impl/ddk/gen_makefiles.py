...
def _gen_makefile(package: str,
  ...
  rel_root = "/".join([".."] * len(package.split("/")))
...
def gen_ddk_makefile(
  ...
  srcs = [pathlib.Path(src[len(package) + 1:]) for src in kernel_module_srcs if
...
if __name__ == "__main__":
  ...
  parser.add_argument("--kernel-module-srcs", type=str, nargs="*")
