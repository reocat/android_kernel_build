...
def _merge_directories(
    output_makefiles: pathlib.Path, submodule_makefile_dir: pathlib.Path
):
    ...
    for root, dirs, files in os.walk(submodule_makefile_dir):
        for file in files:
            ...
            with open(dst_path, "a") as dst, open(submodule_file, "r") as src:
...
def _gen_ddk_makefile_for_module(
    ...
    **unused_kwargs,
    ...
    for kernel_module_srcs_json_item in kernel_module_srcs_json_content:
        ...
        rel_item["files"] = [
            pathlib.Path(src).relative_to(package)
    ...
    with open(kbuild, "w") as out_file, open(
        out_cflags_path, "w"
    ) as out_cflags:
...
def _check_srcs_valid(
    rel_srcs: list[dict[str, Any]], kernel_module_out: pathlib.Path
):
    ...
    source_files_with_name_of_kernel_module = [
        src
        for src in rel_srcs_flat
        if src.with_suffix(".ko") == kernel_module_out
    ]

    if source_files_with_name_of_kernel_module and len(rel_srcs_flat) > 1:
        die(
            "Source files %s are not allowed to build %s when multiple source"
            " files exist. Please change the name of the output file.",
...
def _handle_src(
    ...
    if not src.is_relative_to(kernel_module_out.parent):
        die(
            "%s is not a valid source because it is not under %s",
            src,
            kernel_module_out.parent,
        )
...
def _handle_linux_includes(
    out_file: TextIO, linux_include_dirs: list[pathlib.Path]
):
...
def _handle_defines(out_cflags: TextIO, local_defines: list[str]):
...
def _handle_includes(out_cflags: TextIO, include_dirs: list[pathlib.Path]):
...
def _handle_copts(
    out_cflags: TextIO, copts: Optional[list[dict[str, str | bool]]]
):
    if not copts:
        return
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--kernel-module-srcs-json", type=argparse.FileType("r"), required=True
    )
    parser.add_argument("--output-makefiles", type=pathlib.Path)
    parser.add_argument(
        "--linux-include-dirs", type=pathlib.Path, nargs="*", default=[]
    )
    parser.add_argument(
        "--include-dirs", type=pathlib.Path, nargs="*", default=[]
    )
    parser.add_argument(
        "--module-symvers-list", type=pathlib.Path, nargs="*", default=[]
    )
    ...
    parser.add_argument(
        "--submodule-makefiles", type=pathlib.Path, nargs="*", default=[]
    )
