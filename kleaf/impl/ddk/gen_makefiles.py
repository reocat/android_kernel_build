...
def gen_ddk_makefile(
    ...
    if kernel_module_out.suffix != ".ko":
        die(f"Invalid output: {kernel_module_out}; must end with .ko")
    ...
    with open(kbuild, "w") as out_file:
        ...
        for src in rel_srcs:
            ...
            if src.parent != kernel_module_out.parent:
                die(f"{src} is not a valid source because it is not under {kernel_module_out.parent}")
