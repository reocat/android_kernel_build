...
def _ddk_uapi_headers_impl(ctx):
    ...
    if not ctx.attr.srcs:
        ...
        print("\nWARNING: {}: Empty header list".format(ctx.label))
    ...
    """.format(
        ...
        uapi_headers = " ".join([hdr.path for hdr in ctx.files.srcs]),
...
ddk_uapi_headers = rule(
    ...
    ddk_uapi_headers(
       ...
       kernel_buld = "//common:kernel_aarch64",
    )
    ```
    """,
    attrs = {
