...
def _makefile_impl(ctx):
    ...
    command += """
             # Generate Makefile for DDK module
               {gen_ddk_makefile} --kernel-module-srcs {module_srcs_txt} --kernel-module-outs {module_outs_txt} --output-makefile {makefile} --package {package}
    """.format(
        gen_ddk_makefile = ctx.file._gen_makefile.path,
