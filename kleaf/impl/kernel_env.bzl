...
def _kernel_env_impl(ctx):
    ...
    hermetic_toolchain = hermetic_toolchain_utils.get(ctx)
    ...
    transitive_tools = [hermetic_toolchain.deps]

    toolchains = _get_toolchains(ctx)

    command = hermetic_toolchain.setup
    ...
    setup = hermetic_toolchain.setup
    if ctx.attr._debug_annotate_scripts[BuildSettingInfo].value:
        setup += debug.trap()
    ...
    setup += """
         ...
         # workspace root, making $abs_srctree not under $ROOT_DIR.
           ...
           """.format(
        hermetic_tools_additional_setup = hermetic_toolchain.additional_setup,
        env = out_file.path,
        build_utils_sh = ctx.file._build_utils_sh.path,
...
def _get_run_env(ctx, srcs):
    ...
    setup = hermetic_toolchain.run_setup
    if ctx.attr._debug_annotate_scripts[BuildSettingInfo].value:
        setup += debug.trap()
    ...
    setup += hermetic_toolchain.run_additional_setup
    tools = [
        ctx.file.setup_env,
        ctx.file._build_utils_sh,
    ]
