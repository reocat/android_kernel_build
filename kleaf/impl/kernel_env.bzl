    ...
    command += """
        ...
        # capture it as a file to be sourced in downstream rules
          ( export -p; export -f ) | \\
            ...
            sed "s|\\$PWD/{bin_dir}|__KLEAF_PWD_BIN_DIR_PH__|g" | \\
            sed "s|{bin_dir}|__KLEAF_BIN_DIR_PH__|g" | \\
            sed "s|\\$PWD/out|__KLEAF_OUT_PH__|g" | \\
            sed "s|\\$PWD|\\$KLEAF_REPO_DIR|g" | \\
            sed "s|__KLEAF_OUT_PH__|\\$PWD/out|g" | \\
            sed "s|__KLEAF_PWD_BIN_DIR_PH__|\\$PWD/\\$KLEAF_BIN_DIR|g" | \\
            sed "s|__KLEAF_BIN_DIR_PH__|\\$KLEAF_BIN_DIR|g" \\
            >> {out}
