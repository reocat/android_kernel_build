...
def _get_apply_defconfig_fragments_step(ctx, defconfig_depset_written):
    ...
    if ctx.attr.defconfig_fragments:
        ...
        # merge_cmd may modify .config when defconfig_depset_file has content.
        # If .config may be modified, set need_olddefconfig so _reconfig calls
        # make olddefconfig again.
        cmd += """
...
def _get_check_defconfig_fragments_step(ctx, defconfig_depset_written):
    ...
    if ctx.attr.defconfig_fragments:
        ...
        """.format(
            defconfig_depset_file = defconfig_depset_written.depset_file.path,
            check_defconfig_cmd = config_utils.create_check_defconfig_cmd(
                ctx.label,
                '"${f}"',  # variable in the for loop
            ),
        )
    return struct(
        cmd = cmd,
...
def _reconfig(ctx):
    ...
    defconfig_depset = depset(
        transitive = [target.files for target in ctx.attr.defconfig_fragments],
    )
