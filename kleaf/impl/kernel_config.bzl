...
def _config_symbol_list(ctx):
    ...
    if not ctx.attr.allow_absolute_paths_in_config:
        configs = [
            _config.set_str(
...
def _kernel_config_impl(ctx):
    ...
    if ctx.attr.allow_absolute_paths_in_config and ctx.files.raw_kmi_symbol_list:
        sync_raw_kmi_symbol_list_cmd = """
    ...
    if ctx.attr.allow_absolute_paths_in_config:
        extra_restore_outputs_cmd = """
...
""",
    attrs = {
        ...
        "allow_absolute_paths_in_config": attr.bool(
            doc = "allow absolute paths in .config",
        ),
