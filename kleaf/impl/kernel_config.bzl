...
def _reconfig(ctx):
    ...
    if ctx.files.defconfig_fragments:
        transitive_deps.append(depset(transitive = [target.files for target in ctx.attr.defconfig_fragments]))
        defconfig_fragments_paths = [f.path for f in ctx.files.defconfig_fragments]

        apply_defconfig_fragments_cmd = config_utils.create_merge_dot_config_cmd(" ".join(defconfig_fragments_paths))
        apply_defconfig_fragments_cmd += """
            need_olddefconfig=1
        """
