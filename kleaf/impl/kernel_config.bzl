...
def _config_btf_debug_info(ctx):
    ...
    if btf_debug_info == "default":
        ...
        return struct(configs = [_config.enable("DEBUG_INFO_BTF")], deps = [])
    elif btf_debug_info == "disable":
        return struct(configs = [_config.disable("DEBUG_INFO_BTF")], deps = [])
    else:
        fail("unexpected value for --btf_debug_info: {}".format(btf_debug_info))
