...
def _get_build_number(repository_ctx):
    ...
    build_number = _parse_env(repository_ctx, _BUILD_NUM_ENV_VAR, repository_ctx.attr.apperant_name)
    if not build_number:
...
"""
        content = fmt.format(
            local_filename = repr(_basename(local_filename)),
...
kernel_prebuilt_repo = repository_rule(
    ...
    attrs = {
        ...
        "apperant_name": attr.string(doc = "apperant repo name", mandatory = True),
