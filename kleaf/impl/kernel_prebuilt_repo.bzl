...
def _get_build_number(repository_ctx):
    ...
    build_number = _parse_env(repository_ctx, _BUILD_NUM_ENV_VAR, repository_ctx.attr.apparant_name)
    if not build_number:
...
kernel_prebuilt_repo = repository_rule(
    ...
    attrs = {
        ...
        "apparant_name": attr.string(doc = "apparant repo name", mandatory = True),
