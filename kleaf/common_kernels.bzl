...
def _default_kmi_configs():
    ...
    return {
        "kernel_aarch64": {
            ...
            "trim_nonlisted_kmi": "default_true" if len(native.glob(["android/abi_gki_aarch64*"])) > 0 else None,
        },
        "kernel_aarch64_debug": {
            "kmi_symbol_lists": native.glob(["android/abi_gki_aarch64*"]),
...
def _define_kernel_build_and_notrim(
        name,
        visibility,
    ...
    |                                 |--trim=true|--trim=false|--trim=default              |
    :                                 :           :            :(use value in `kmi_configs`):
    |---------------------------------|-----------|------------|----------------------------|
    |kernel_aarch64                   |TRIM       |NO TRIM     |TRIM                        |
    :(with symbol lists)              :           :            :                            :
    :(trim_nonlisted_kmi=default_true):           :            :                            :
    :                                 :           :            :                            :
    |kernel_aarch64                   |FAIL       |NO TRIM     |NO TRIM                     |
    :(no symbol lists)                :           :            :                            :
    :(trim_nonlisted_kmi=None)        :           :            :                            :
    :                                 :           :            :                            :
    ...
    :                                 :           :            :                            :
    :                                 :           :            :                            :
    |kernel_x86_64                    |FAIL       |NO TRIM     |NO TRIM                     |
    :(trim_nonlisted_kmi=None)        :           :            :                            :
    :                                 :           :            :                            :
    ...
    native.alias(
        name = name + "_notrim",
        actual = select({
            ":internal_trim_" + cmdline_value: _select_notrim_target(name, trim_nonlisted_kmi, cmdline_value)
...
def define_common_kernels(
    ...
    **Prebuilts**
      ...
      kmi_configs: A dictionary, where keys are target names, and
        ...
        ```
        {
            "kernel_aarch64": {
                ...
                "trim_nonlisted_kmi": "default_true" if len(glob(["android/abi_gki_aarch64*"])) > 0 else None,
            },
            "kernel_aarch64_debug": {
                "kmi_symbol_lists": glob(["android/abi_gki_aarch64*"]),
                trim_nonlisted_kmi: "default_false",
            },
        }
        ```
    ...
    for name, arch_config in _ARCH_CONFIGS.items():
        ...
        _define_kernel_build_and_notrim(
            name = name,
            srcs = [name + "_sources"],
    ...
    _define_trim_config_settings()
    _define_prebuilts(visibility = visibility)

def _define_trim_config_settings():
    for trim_value in TRIM_CMDLINE_VALID_VALUES:
