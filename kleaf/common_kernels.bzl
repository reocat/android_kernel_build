...
def define_common_kernels(
    ...
    **Prebuilts**
    ...
    Args:
      ...
      To print the actual configurations for debugging purposes (assuming
      that the current package is `//common`):

      ```
      bazel build //common:kernel_configs
      ```
    ...
    print_debug(
        ...
        content = json.encode_indent({
            name: target_configs[name]
            for name in _ARCH_CONFIGS.keys()
        }, indent = "    ").replace("null", "None"),
    )
