...
def define_common_kernels(
    ...
    `<name>_notrim` is similar to `<name>`, except that `trim_nonlisted_kmi` is
    always `False`. The target is meant to be used as the `base_kernel` of
    the device [`kernel_build`](#kernel_build) **during device bring-up** for
    debugging purposes.
    Prior to release, all references to `<name>_notrim` should be changed to
    `<name>` to avoid KMI violation.
