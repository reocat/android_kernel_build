    ...
    for out in outputs:
        ...
        for sdir in srcdir:
            if os.path.exists(os.path.join(sdir, out)):
                ...
                os.makedirs(
                    os.path.dirname(os.path.join(dstdir, out)), exist_ok=True
                )
        ...
        for sdir in srcdir:
            ...
            if not found:
                ...
                if len(matches) > 1:
                    found = True
                    errors.append(
                        f"In {sdir}, multiple files match '{out}', expected at"
                        " most 1:\n  "
                        + "\n  ".join(matches)
                    )
                    break
...
def search_and_cp_output_one(srcdir, dstdir, out):
    ...
    For each output in <outputs>, searches <output> within <srcdir>, and moves
    it
