...
# First, .source_date_epoch_dir might be a device kernel source tree, so
# look for setlocalversion there.
# If that does not exist, look for setlocalversion in the ACK tree.
SETLOCALVERSION=""
if [[ "$KERNEL_DIR" ]] && [[ -x "$KERNEL_DIR/scripts/setlocalversion" ]]; then
  SETLOCALVERSION=$(readlink -f "$KERNEL_DIR/scripts/setlocalversion")
elif [[ -x "common/scripts/setlocalversion" ]]; then
  SETLOCALVERSION=$(readlink -f "common/scripts/setlocalversion")
fi

STABLE_SCMVERSION=
if [[ "$SETLOCALVERSION" ]] && [[ "$KERNEL_DIR" ]] && [[ -d "$KERNEL_DIR" ]]; then
