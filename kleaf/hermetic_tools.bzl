...
e.g. for generated executables and tests""",
        "test_tools_short_path": """Value of `PATH` to be set before running tests.

This is a relative
[`short_path`](#https://docs.bazel.build/versions/main/skylark/lib/File.html#short_path);
caller should `realpath` it before using.""",
    },
)
            ...
            link_one_cmd = """
                (
                    ...
                    if [[ -n "$actual" ]]; then
                        ...
                        {hermetic_base}/cat {no_host_tool} $i
                    fi
                )
            """.format(
...
""".format(path = paths.dirname(all_outputs[0].short_path))
    ...
    return [
        ...
        HermeticToolsInfo(
            ...
            run_setup = run_setup,
            test_deps = test_deps,
            test_tools_short_path = all_outputs[0].short_path,
        ),
    ]
