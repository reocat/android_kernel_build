...
_DEFAULT_KERNEL_BUILD_SRCS = """glob(["**"],\\ exclude=["**/.*",\\ "**/.*/**",\\ "**/BUILD.bazel",\\ "**/*.bzl",])"""
...
def find_build_config(env: Mapping[str, str]) -> str:
    ...
    if env.get("BUILD_CONFIG"):
        ...
        if os.path.commonpath([real_build_config, real_this]) != real_this:
            die(
                f"realpath $BUILD_CONFIG ({real_build_config}) is not under the"
                " repository root"
            )
        return os.path.relpath(real_build_config, real_this)
    die(
        "$BUILD_CONFIG is not set, and top level build.config file is not"
        " found."
    )
...
def infer_target_name(args, build_config: str) -> str:
    ...
    if (
        build_config_base.startswith(_BUILD_CONFIG_PREFIX)
        and build_config_base != _BUILD_CONFIG_PREFIX
    ):
...
class BuildConfigToBazel(buildozer_command_builder.BuildozerCommandBuilder):
    ...
    def __init__(self, *init_args, **init_kwargs):
        ...
        self.new_env = order_dict_by_key(
            json.loads(
                subprocess.check_output(
                    (
                        "source build/kernel/_setup_env.sh > /dev/null &&"
                        " build/kernel/kleaf/dump_env.py"
                    ),
                    shell=True,
                    stderr=self.stderr,
                    env=self.environ,
        ...
        self.unstripped_modules_name = (
            f"{target_name}_unstripped_modules_archive"
        )
    ...
    def _create_buildozer_commands(self) -> None:
        ...
        dist = self._new(
            "copy_to_dist_dir",
            self.dist_name,
            load_from="//build/bazel_common_rules/dist:dist.bzl",
            add_to_dist=False,
        )
            ...
            if key in _IGNORED_BUILD_CONFIGS:
                if not re.match(_IGNORED_BUILD_CONFIGS[key], value):
                    target_comment.append(
                        f"FIXME: {key}={esc_value} not supported"
                    )
                continue
                # else ignore
                ...
                self._set_attr(
                    target, "build_config", os.path.basename(value), quote=True
                )
            elif key == "BUILD_CONFIG_FRAGMENTS":
                target_comment.append(
                    f"FIXME: {key}={esc_value}: Please manually convert to"
                    " kernel_build_config"
                )
            elif key == "FAST_BUILD":
                if value:
                    target_comment.append(
                        f"FIXME: {key}: Specify --config=fast in device.bazelrc"
                    )
            elif key == "LTO":
                if value:
                    target_comment.append(
                        f"FIXME: {key}: Specify --lto={value} in device.bazelrc"
                    )
            elif key == "DIST_DIR":
                rel_dist_dir = os.path.relpath(value)
                self._add_comment(
                    dist, "dist_dir", f'FIXME: or dist_dir = "{rel_dist_dir}"'
                )
                ...
                for module_package in module_packages:
                    ...
                    self._add_comment(
                        ...
                        "temp_outs",
                        (
                            "FIXME: set to the list of external modules in"
                            " this package. You may run `tools/bazel build"
                            f" {module}` and follow the instructions in the"
                            " error message."
                        ),
                ...
                if value != self.package:
                    if value.removesuffix("/") == common:
                        ...
                        self._add_attr(
                            target,
                            "srcs",
                            f"//{common}:kernel_aarch64_sources",
                            quote=True,
                        )
                    else:
                        self._add_comment(
                            ...
                            "srcs",
                            (
                                "FIXME: add files from KERNEL_DIR"
                                f" {self.new_env['KERNEL_DIR']}"
                            ),
                        )
                ...
                self._set_attr(
                    target, "collect_unstripped_modules", bool(value)
                )
                ...
                self._add_comment(
                    ...
                    "abi_definition",
                    (
                        "Usually not set in Kleaf. See"
                        " build/kernel/kleaf/docs/abi_device.md. Original"
                        f" value: //{common}:{value}"
                    ),
                    lambda attr_val: attr_val.is_missing_or_none(),
                )
                ...
                self._set_attr(
                    target, "kmi_symbol_list", f"//{common}:{value}", quote=True
                )
                ...
                target_comment.append(
                    "FIXME: PRE_DEFCONFIG_CMDS: Don't forget to modify"
                    " PRE_DEFCONFIG_CMDS "
                ...
                self._set_attr(
                    images, key.removesuffix("_IMG").lower(), bool(value)
                )
                ...
                modules_options_path = os.path.join(
                    self.package, modules_options_filename
                )
                ...
                if os.path.commonpath((value, self.package)) == self.package:
                    self._set_attr(
                        images,
                        key.lower(),
                        os.path.relpath(value, start=self.package),
                        quote=True,
                    )
                else:
                    self._add_comment(
                ...
                if value == f"{common}/build.config.gki.aarch64":
                    ...
                    self._add_comment(
                        ...
                        "base_kernel",
                        (
                            "FIXME: set base_kernel to kernel_build for"
                            f" {esc_value}"
                        ),
                    )
                ...
                if self.environ[key] == value:
                    ...
                    target_comment.append(
                        f"FIXME: Unknown in build config: {key}={esc_value}"
                    )
                    unknowns.append(key)
            else:
                target_comment.append(
        ...
        if need_unstripped_modules or abi:
            unstripped_modules = self._new(
                "kernel_unstripped_modules_archive",
                self.unstripped_modules_name,
            )
            self._set_attr(
                unstripped_modules, "kernel_build", target, quote=True
            )
            for module in modules:
                self._add_attr(
                    unstripped_modules, "kernel_modules", module, quote=True
                )

        modules_install = None
        need_modules_install = images or modules
        if need_modules_install:
            modules_install = self._new(
                "kernel_modules_install", self.modules_install_name
            )
            self._set_attr(modules_install, "kernel_build", target, quote=True)
            for module in modules:
                self._add_attr(
                    modules_install, "kernel_modules", module, quote=True
                )
        ...
        if images:
            ...
            self._set_attr(
                images, "kernel_modules_install", modules_install, quote=True
            )
        ...
        self._add_comment(
            ...
            "base_kernel",
            (
                "FIXME: base_kernel should be migrated to"
                f" //{common}:kernel_aarch64."
            ),
            lambda attr_val: attr_val.value
            not in (f"//{common}:kernel_aarch64", f"//{common}:kernel"),
        )
        ...
        self._add_comment(
            ...
            "module_outs",
            (
                "FIXME: set to the list of in-tree modules. You may run "
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        "--target",
        help=(
            "Name of target. Otherwise, infer from the name of the "
            "build.config file."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "--keep-going",
        help=(
            "buildozer keeps going on errors. Use when targets are already "
