...
""")
    ...
    def _create_buildozer_commands(self) -> None:
        ...
        for key, value in self.new_env.items():
            ...
            if type(self)._is_ignored_build_config(key):
                ...
                images = self._new("kernel_images", self.images_name)
                self._set_attr(images, key.lower(), bool(value == "1"))
        ...
        self._add_target_comment(dist, dist_comment)
        self._add_target_comment(images, images_comment)
    ...
    def run(self):
        with self as builder:
            # Dry run to see what attributes / targets will be added
            self.existing = dict()
            builder._create_buildozer_commands()

            # Get the existing information in BUILD files, and rerun
            self.existing = self._get_all_info(self.existing.keys())

        with self as builder:
            builder._create_buildozer_commands()
            builder._run_buildozer()
