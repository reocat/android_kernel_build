...
The skelseton Bazel BUILD file likely won't build properly. Manual intervention
...
# Variables not supported by Kleaf. Device owners will need to migrate away
...
class BuildozerCommandBuilder(object):
    def __init__(self, args, stdout=None, stderr=None, environ=None):
        ...
        self.old_env: dict[str, str] = order_dict_by_key(self.environ)
...
""")
    ...
    def _create_extra_file(self, path, content):
        """Create an extra file in the filesystem."""
        if self.args.stdout:
    ...
    def _create_buildozer_commands(self) -> None:
        ...
        for key, value in self.new_env.items():
            ...
            if type(self)._is_ignored_build_config(key):
                ...
                target_comment.append(
                    f"FIXME: PRE_DEFCONFIG_CMDS: Don't forget to modify PRE_DEFCONFIG_CMDS so it writes to $OUT_DIR, not the source tree: https://android.googlesource.com/kernel/build/+/refs/heads/master/kleaf/docs/errors.md#defconfig-readonly")
            elif key in (
                    "BUILD_BOOT_IMG",
...
def parse_args(argv):
    ...
    parser.add_argument("-k",
                        help="buildozer keeps going on errors. Use when targets are already defined. There may be duplicated FIXME comments.",
                        action="store_true")
