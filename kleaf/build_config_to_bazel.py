  ...
  def _create_buildozer_commands(self) -> None:
      ...
      if key in _IGNORED_BUILD_CONFIGS:
        ...
        for module_package in module_packages:
          ...
          self._add_comment(
              ...
              "temp_outs",
              (
                  "FIXME: set to the list of external modules in this package."
                  f" You may run `tools/bazel build {module}` and follow the"
        ...
        if value != self.package:
          if value.removesuffix("/") == common:
            ...
            self._add_attr(
                target, "srcs", f"//{common}:kernel_aarch64_sources", quote=True
            )
          else:
            self._add_comment(
                target,
        ...
        images = self._new("kernel_images", self.images_name)
        self._add_comment(
            images, "mkbootimg", f"FIXME: set mkbootimg to label of {esc_value}"
        )
        ...
        if value == f"{common}/build.config.gki.aarch64":
          self._set_attr(
              target, "base_kernel", f"//{common}:kernel_aarch64", quote=True
          )
        else:
          self._add_comment(
              target,
        ...
        dts = self._new(
            "kernel_dtstree", self.dts_name, package=value, add_to_dist=False
        )
    ...
    if need_unstripped_modules or abi:
      unstripped_modules = self._new(
          "kernel_unstripped_modules_archive", self.unstripped_modules_name
      )
    ...
    if abi:
      ...
      self._set_attr(
          abi, "unstripped_modules_archive", unstripped_modules, quote=True
      )
