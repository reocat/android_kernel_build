...
function build_system_dlkm() {
  ...
  tar -czf "${DIST_DIR}/system_dlkm_staging_archive.tar.gz" --mtime '1970-01-01' -C "${SYSTEM_DLKM_STAGING_DIR}" .
}
...
function build_gki_boot_images() {
  ...
  tar -czf "${DIST_DIR}/${GKI_BOOT_IMG_ARCHIVE}" --mtime '1970-01-01' -C "${DIST_DIR}" \
    "${images_to_pack[@]}"
}
