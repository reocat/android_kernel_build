...
# get_boot_img_size /path/to/boot.img.
# The function echoes the value of the preconfigured size variable
# based on the input file name. e.g.,
#   - boot-5.10.img: echo ${BOOT_IMG_SIZE}
#   - boot-5.10-gz.img: echo ${BOOT_IMG_GZ_SIZE}
#   - boot-5.10-lz4.img: echo ${BOOT_IMG_LZ4_SIZE}
function get_boot_img_size() {
  local boot_img
  local file_name
  local compression

  boot_img=$(basename "$1")
  if [[ "${boot_img}" =~ boot-[A-Za-z0-9.]+.img ]]; then
    boot_size_var="BOOT_IMG_SIZE"
  else
    file_name="${boot_img%.img}"
    compression=$(echo "${file_name##boot*-}" | tr '[:lower:]' '[:upper:]')
    boot_size_var="BOOT_IMG_${compression}_SIZE"
  fi
...
function gki_add_avb_footer() {
  ...
  partition_size=$(get_boot_img_size "${boot_img}")
