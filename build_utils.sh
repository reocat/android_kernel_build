...
# $3 kernel version
# $4 Optional: list of modules to run depmod on. If left empty, depmod will run
#    on all modules under $1/lib/modules/x.y
function run_depmod() {
  (
    local ramdisk_dir=$1
    local depmod_stdout
    ...
    cd ${ramdisk_dir}
    if ! depmod_stdout="$(depmod $2 -F ${DIST_DIR}/System.map -b . $3 $4\
...
# $2 flags to pass to depmod
# $3 kernel version
# $4 file containing a list of modules to run depmod on
function run_depmod_for_mode() {
  local ramdisk_dir=$1
  local depmod_flags=$2
  local version=$3
  ...
  run_depmod ${ramdisk_dir} "${depmod_flags}" "${version}" "${modules_list}"
}
...
# $5 MODULES_OUTPUT_FILENAME <The name of the output modules.order file>
function create_additional_modules_order() {
  ...
  cat ${modules_list_file} > ${tmp_all_modules_list_file}
  # Append to the first stage init modules
  grep -v "^\#" ${additional_modules_file} >> ${tmp_all_modules_list_file}
  grep -w -f ${tmp_all_modules_list_file} ${modules_order_file} > ${dest_dir}/${modules_out_filename}
...
function create_modules_staging() {
  ...
  if [[ -f ${dest_dir}/modules.order.recovery ]]; then
    run_depmod_for_mode ${dest_stage} "${depmod_flags}" "${version}" \
                        "${dest_dir}/modules.order.recovery"
    cp ${dest_dir}/modules.order.recovery ${dest_dir}/modules.load.recovery
  fi

  if [[ -f ${dest_dir}/modules.order.charger ]]; then
    run_depmod_for_mode ${dest_stage} "${depmod_flags}" "${version}" \
                        "${dest_dir}/modules.order.charger"
