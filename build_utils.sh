...
function create_modules_staging() {
  ...
  rm -rf ${dest_dir}
  mkdir -p ${dest_dir}
  local in_tree_modules_list=$(mktemp -t in_tree_modules.XXXXXX)
  pushd ${src_dir}
    find . -path "./extra/*" -prune -o -name "*.ko" -print > ${in_tree_modules_list}
  popd
  while read m; do
    mkdir -p ${dest_dir}/$(dirname $m)
    cp ${src_dir}/$m ${dest_dir}/$m
  done < ${in_tree_modules_list}
  rm -f ${in_tree_modules_list}
...
function make_dtbo() {
  ...
  (
    ...
    if [ -f "${DIST_DIR}/dtbo.img" ]; then
      echo "dtbo image created at ${DIST_DIR}/dtbo.img"
    fi
  )
}
