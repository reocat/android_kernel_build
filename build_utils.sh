...
function filter_module_list() {
  local modules_list_file="${1}"
  local modules_order_list="${2}"
  local dest_file="${3}"
  ...
  ! grep -w -f ${modules_list_filter} ${modules_order_list} > ${dest_file}
...
function create_modules_staging() {
  ...
  for mod_order_file in ${!module_lists_arr[@]}; do
    local mod_list=${module_lists_arr[${mod_order_file}]}
    local dest_file=${dest_dir}/${mod_order_file}

    if [ -n "${mod_list}" ] && ! filter_module_list ${mod_list} ${old_modules_list} ${dest_file}; then
        exit 1
    fi
  done

  rm -f ${old_modules_list}
