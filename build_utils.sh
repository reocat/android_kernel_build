...
# $4 IMAGE_STAGING_DIR <The destination directoryin which MODULES_LIST is
...
function create_recovery_modules_order() {
  ...
  if [[ -f "${ROOT_DIR}/${modules_recoverylist_file}" ]]; then
    ...
    echo "modules recovery list must be an absolute path or relative to ${ROOT_DIR}: ${modules_recoverylist_file}"
    exit 1
  elif [[ ! -f "${modules_recoverylist_file}" ]]; then
    echo "Failed to find modules list: ${modules_recoverylist_file}"
    exit 1
  fi
...
# $6 MODULES_RECOVERY_LIST <File contains the list of modules that should go in
...
function create_modules_staging() {
  ...
  local modules_recoverylist_file=$6
