...
# build_boot_images_for_kernels <uncompressed kernel path>.
...
function build_boot_images_for_kernels() {
...
function build_gki_artifacts() {
  ...
  if [ "${ARCH}" = "arm64" ]; then
    build_boot_images_for_kernels "${DIST_DIR}/Image"
  elif [ "${ARCH}" = "x86_64" ]; then
    build_boot_images_for_kernels "${DIST_DIR}/bzImage"
  else
